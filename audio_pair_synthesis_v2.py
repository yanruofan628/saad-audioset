...
def load_features_data(features_path):
    ...
    print(f"开始从各子类别目录加载特征数据...")
    
    for sub_cat in all_subcategories:
        # 构建特征文件路径
        feature_file = os.path.join(features_path, sub_cat, f"{sub_cat}_features_summary.csv")
        
        if os.path.exists(feature_file):
            try:
                # 读取CSV文件
                df = pd.read_csv(feature_file)
                print(f"成功加载 {sub_cat} 的特征数据，包含 {len(df)} 个文件")
                ...
                for _, row in df.iterrows():
                    ...
                    file_path = os.path.join(features_path, sub_cat, file_name)
                    ...
                    feature_info = {
                        ...
                        'file_path': file_path,
                        'features': {
                            'feature_matrix_normalized': None,  # 这里需要根据实际CSV结构调整
                            'variation_sum': row.get('variation_sum', 0)
                        }
                    }
...
def nearest_neighbor_matching(features_data, category_mapping, used_files, output_dir):
    ...
    for left_idx, (left_main_cat, left_sub_cat) in enumerate(all_subcategories):
        ...
        for right_idx, (right_main_cat, right_sub_cat) in enumerate(all_subcategories):
            ...
            # 由于现在没有预存的特征矩阵，我们使用variation_sum作为简单的相似度指标
            left_variation = left_file_info['features']['variation_sum']
            ...
            for right_file_info in audio_queues[(right_main_cat, right_sub_cat)]:
                # 获取右声道音频的variation_sum
                right_variation = right_file_info['features']['variation_sum']
                
                # 使用variation_sum的相似度（差值越小越相似）
                similarity = 1.0 / (1.0 + abs(left_variation - right_variation))
...
if __name__ == "__main__":
    # 设置路径
    features_path = r"D:\D\research\audioset下载\classified_audio"  # 包含各子类别目录的路径
