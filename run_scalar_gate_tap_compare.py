...
"""
对比：
  residual_scalar_gate      — 标准 EEGNet AdaptiveAvgPool
  residual_scalar_gate_tap  — Temporal Attention Pooling（1×1 score，替换时间 AvgPool）

环境变量：SUBJECTS, TRAIN_BATCH_SIZE
"""
...
def train_one_model(name, fold_data, am_fm, device, batch_size):
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        for b in range(n_batches):
            ...
            logits, delta, p_l, p_r = out[0], out[2], out[3], out[4]
            loss = (
                crit(logits, yb)
                + LAMBDA_REG * torch.norm(delta, p=2, dim=1).mean()
    ...
    with torch.no_grad():
        out = model(
            X_val, am_l_v, am_r_v, mel_l_v, mel_r_v, s_l_v, s_r_v, return_parts=True
        )
        logits = out[0]
        attn = out[6]
        val_pred = logits.argmax(1).cpu().numpy()
    ...
    if attn is not None and 'tap' in name:
        # peak latency proxy on T' (relative index)
        a = attn.mean(0).detach().cpu().numpy()
        result[f'attn_peak_bin_{name}'] = float(np.argmax(a))
        result[f'attn_entropy_{name}'] = float(-(a * np.log(a + 1e-8)).sum())
    return result
...
def main():
    ...
    tag = f'scalar_gate_tap_compare_mirror_bs{batch_size}'
