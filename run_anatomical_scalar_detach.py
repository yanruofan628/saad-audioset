...
# -*- coding: utf-8 -*-
"""
category_heldout：解剖锚定 + 标量 gate 残差（detach 训练）。
D = S + g·B；训练时 D_train = S.detach() + g·B，residual/gate 读 detached audio。

默认 6 人；输出：.../individual/category_heldout_anat_scalar_detach_v1/
"""
...
def run_subject(subject_name, am_fm, pair_name_to_idx, type_map, out_dir, device):
    ...
    for fold_idx, val_idx in enumerate(folds, 1):
        ...
        print(
            f"  Fold {fold_idx}: audio BACC={m['val_balanced_accuracy_audio']:.4f} | "
            f"fusion BACC={m['val_balanced_accuracy_fusion']:.4f} | "
            f"g_mean={m['val_gate_mean']:.3f}"
        )

    os.makedirs(out_dir, exist_ok=True)
    df = pd.DataFrame(fold_rows)
    csv_path = os.path.join(out_dir, f"{subject_name}_scalar_detach_cv_results.csv")
    ...
    summary = (
        ...
        f"audio (S) BACC: {b_a:.4f}\n"
        f"fusion D=S+gB (detach train) BACC: {b_f:.4f}\n"
        f"delta (fusion-audio): {b_f - b_a:+.4f}\n"
    )
    with open(
        os.path.join(out_dir, f"{subject_name}_scalar_detach_cv_summary.txt"),
        "w",
        encoding="utf-8",
    ) as f:
        f.write(summary)
...
def main():
    ...
    print(f"LAMBDA_FUSION={LAMBDA_FUSION}")
    ...
    out_dir = os.path.join(OUTPUT_DIR, "individual", "category_heldout_anat_scalar_detach_v1")
    os.makedirs(out_dir, exist_ok=True)
    ...
    if rows:
        ...
        summary_name = f"n{len(rows)}_scalar_detach_summary.csv"
