...
SUBJECTS_CONFIG = {
    ...
    'hehaohuai': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/hehaohuai',
        'mff_folders': [
    ...
    'qiuhaiyun': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/qiuhaiyun',
        'mff_folders': [
    ...
    'zhouyu': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/zhouyu',
        'mff_folders': [
    ...
    'honghaokai': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/honghaokai',
        'mff_folders': [
    ...
    'caolulu': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/caolulu',
        'mff_folders': [
    ...
    'yanyinsong': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/yanyinsong',
        'mff_folders': [
    ...
    'huangxiaohang': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/huangxiaohang',
        'mff_folders': [
    ...
    'xufan': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/xufan',
        'mff_folders': [
    ...
    'qiusiqi': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/qiusiqi',
        'mff_folders': [
    ...
    'machenxiang': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/machenxiang',
        'mff_folders': [
    ...
    'lizhuhang': {
        'processed_data_path': 'A:/standard_data_interp_no_ica/lizhuhang',
        'mff_folders': [
...
def train_fold_model(fold_data, all_audio_features, pair_name_to_idx, pair_names, all_sync_features=None):
    """
    在单个fold上训练模型并返回评估结果
    ...
    # 创建模型：simple / audio_only / residual / residual_sync（如果启用同步性）
    models = {
        ...
        # 原始 residual 基线（端到端 EEGNet + 残差）
        'residual': ResidualFusionClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim
        ),
        # 仅基于 EEG 残差的 baseline（不含同步性），使用新的 EEG 提取器实现
        'residual_eeg_only': ResidualEEGFusionClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim
        ),
        # 方案 A：单一全局 audio query → EEG 时间序列 cross-attention
        'residual_eeg_cross_single': ResidualEEGFusionCrossAttnSingleQuery(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim
        ),
        # 方案 B：左右耳双 query → EEG 时间序列 cross-attention
        # 这里暂时假设左右耳特征可以由 audio 特征在外部拆分/映射得到，因此 ear_feature_dim 暂设为 audio_dim // 2
    ...
    # 如果启用同步性特征，添加 residual_sync 模型
    if use_sync:
        models['residual_sync'] = ResidualSyncFusionClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim,
            sync_dim=sync_dim
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            for name, model in models.items():
                ...
                if name in ('residual', 'residual_sync'):
                    ...
                    print(f"    Epoch {epoch + 1}/{n_epochs} [{name:12s}] Train Loss: {tl:.4f}, Train Acc: {ta:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}")
    ...
    with torch.no_grad():
        for name, model in models.items():
            ...
            print(f"    {name:12s} - 准确率: {acc:.4f}, 平衡准确率: {bal_acc:.4f}, AUC: {auc:.4f}, Epochs: {actual_epochs}")
...
def main(filter_consistent=True):
    ...
    for fold_idx, val_global_indices in enumerate(folds, 1):
        ...
        # 打印结果（包含 residual_sync 如果存在）
        result_str = (
            f"  Fold {fold_idx} 结果: "
            f"simple={fold_metrics['val_accuracy_simple']:.4f}, "
            f"audio_only={fold_metrics['val_accuracy_audio_only']:.4f}, "
            f"residual={fold_metrics['val_accuracy_residual']:.4f}"
        )
        if 'val_accuracy_residual_sync' in fold_metrics:
            result_str += f", residual_sync={fold_metrics['val_accuracy_residual_sync']:.4f}"
        print(result_str)
    ...
    model_names = ['simple', 'audio_only', 'residual']
    # 检查是否有 residual_sync 的结果
    if any('val_accuracy_residual_sync' in result for result in all_fold_results):
        model_names.append('residual_sync')
