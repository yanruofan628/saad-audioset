    ...
    for sub_cat in all_subcategories:
        ...
        if os.path.exists(category_dir) and os.path.exists(clap_category_dir):
            try:
                ...
                print(f"  特征目录中的文件: {all_files}")
                
                # 列出5秒音频目录中的所有文件
                clap_files = os.listdir(clap_category_dir)
                print(f"  5秒音频目录中的文件: {clap_files}")
                ...
                for file_name in all_files:
                    if file_name.endswith('_features.npy'):
                        ...
                        for ext in audio_extensions:
                            ...
                            if os.path.exists(audio_1_path):
                                audio_files.append({
                                    'name': audio_1_name,
                                    'path': audio_1_path,
                                    'type': 'first_half'  # 前5秒，取特征的前一半
                                })
                                print(f"  找到前5秒音频: {audio_1_name}")
                            ...
                            if os.path.exists(audio_2_path):
                                audio_files.append({
                                    'name': audio_2_name,
                                    'path': audio_2_path,
                                    'type': 'second_half'  # 后5秒，取特征的后一半
                                })
                                print(f"  找到后5秒音频: {audio_2_name}")
                        ...
                        if audio_files:
                            ...
                            # 加载元数据
                            metadata_path = os.path.join(category_dir, original_audio_name + '_metadata.npy')
                            metadata = None
                            ...
                            for audio_info in audio_files:
                                ...
                                variation_sum = 0
                                if audio_feature_matrix.shape[1] > 1:  # 确保有多个时间帧
                                    for t in range(audio_feature_matrix.shape[1] - 1):
                                        diff = audio_feature_matrix[:, t+1] - audio_feature_matrix[:, t]
                                ...
                                print(f"  ✓ 成功加载: {audio_info['name']} ({audio_info['type']})")
                        else:
                            print(f"  ✗ 未找到对应的5秒音频文件: {original_audio_name}")
            ...
            print(f"未找到 {sub_cat} 的目录: {category_dir} 或 {clap_category_dir}")
            features_data['categories'][sub_cat] = {'features': [], 'count': 0}
    
    total_categories = len([cat for cat in features_data['categories'].values() if cat['count'] > 0])
    print(f"5秒音频特征数据加载完成，包含 {total_categories} 个有效类别")
    ...
    for left_idx, (left_main_cat, left_sub_cat) in enumerate(all_subcategories):
        ...
        for right_idx, (right_main_cat, right_sub_cat) in enumerate(all_subcategories):
            ...
            if left_audio is not None and right_audio is not None:
                ...
                # 合成双声道音频（确保5秒长度）
                success = create_stereo_audio(left_audio, right_audio, output_path, sr, target_duration=5.0)
                if success:
...
def save_pairing_results(pairs_created, output_dir):
    """保存配对结果"""
    # 保存为JSON文件
    json_path = os.path.join(output_dir, "pairing_results_5s.json")
    ...
    csv_path = os.path.join(output_dir, "pairing_results_5s.csv")
...
if __name__ == "__main__":
    ...
    clap_audio_path = r"D:\D\research\audioset下载\clap"  # 5秒音频文件路径
    output_dir = r"D:\D\research\audioset下载\音频配对合成_5s"  # 输出目录
    
    # 运行主函数
    main(features_path, clap_audio_path, output_dir)
    #@random_select_audio.py 再写一个随机挑选的，这次最近邻和随机的都在这里D:\D\research\audioset下载\音频配对合成_5s   我需要你从最近邻随机抽取112条，随机匹配的随机抽取112条，打乱放到一个文件夹里。剩下的所有的打乱放到一个文件夹里