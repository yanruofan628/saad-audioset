...
# -*- coding: utf-8 -*-
"""
全员跑 residual_gated 基线（无 ERSP，标准 EEGNet），与 early_ersp 同设定对比。
被试列表与 run_ersp_early_all_subjects.py 对齐（17 人）。
"""
...
# 与 early_ersp 全员实验同一批人
SUBJECTS = [
    'yanxingzhuo', 'jinxiaoyue', 'chenxianwei', 'yeziyuan', 'zhangzhiyao',
    'haoxiang', 'hehaohuai', 'qiuhaiyun', 'zhouyu', 'honghaokai', 'caolulu',
    'yanyinsong', 'huangxiaohang', 'xufan', 'qiusiqi', 'machenxiang', 'lizhuhang',
]

MODEL_NAME = 'residual_gated'
EEG_MODE = 'default'
LAMBDA_REG = 0.05
N_EPOCHS = 100
PATIENCE = 15
...
def train_one_fold(fold_data, am_fm, device):
    ...
    model = ResidualGatedAMFMBaseline(
...
def run_subject(subject_name, am_fm, pair_name_to_idx, output_dir, device):
    ...
    pd.DataFrame(fold_results).to_csv(
        os.path.join(output_dir, f'{subject_name}_residual_cv_results.csv'),
...
def main():
    subjects = [s for s in SUBJECTS if s in SUBJECTS_CONFIG]
    print('=' * 72)
    print(f'全员基线（无 ERSP）: {MODEL_NAME}')
    ...
    out_root = os.path.join(
        OUTPUT_DIR, 'individual', 'gated_baseline_all', MODEL_NAME, 'mirror'
    )
