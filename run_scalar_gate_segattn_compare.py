...
对比（additive，不改旧模型）：
  ...
  residual_scalar_gate_segattn  — 同上，但 EEG 用 4 窗段级 attention 读出 B
...
def train_one_model(name, fold_data, am_fm, device, batch_size):
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        with torch.no_grad():
            out = model(
                X_val, am_l_v, am_r_v, mel_l_v, mel_r_v, s_l_v, s_r_v,
                return_parts=True,
            )
            logits = out[0]
            attn = out[6]
            ...
            if attn is not None:
                last_alpha = attn[0].detach().cpu().numpy()
        acc = accuracy_score(val_y, val_pred)
        if epoch == 1 or epoch % 10 == 0 or epoch == N_EPOCHS:
            alpha_msg = ''
            if last_alpha is not None and name.endswith('segattn'):
                alpha_msg = (
                    f" α=["
                    + ','.join(f'{a:.2f}' for a in last_alpha.tolist())
                    + ']'
                )
            print(
    ...
    if attn is not None and name.endswith('segattn'):
        alpha = attn[0].detach().cpu().numpy()
        for i, a in enumerate(alpha.tolist()):
            result[f'alpha_seg{i}_{name}'] = float(a)
    return result
...
def run_subject(subject_name, am_fm, pair_name_to_idx, output_dir, device, batch_size):
    ...
    for name in MODELS:
        ...
        if name.endswith('segattn'):
            alphas = []
            for i in range(4):
                key = f'alpha_seg{i}_{name}'
                if key in fold_results[0]:
                    vals = [r[key] for r in fold_results]
                    alphas.append(np.mean(vals))
            if alphas:
                lines.append(
                    f'  mean_alpha(early/mid/late/tail)='
                    + ','.join(f'{a:.3f}' for a in alphas)
                )
                print(
                    f"  mean α early/mid/late/tail: "
                    + ', '.join(f'{a:.3f}' for a in alphas),
                    flush=True,
                )
...
def main():
    ...
    tag = f'scalar_gate_segattn_compare_mirror_bs{batch_size}'
