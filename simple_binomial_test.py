...
class SimpleBinomialTestAnalyzer:
    """简化的二项假设检验分析器"""
    ...
    def create_simple_visualization(self, combined_data, test_results, output_dir):
        """
        创建简化的可视化图表
...
结论: {'拒绝H0，选择比例显著偏离0.5' if test_results['significant_two_tailed'] else '不拒绝H0，选择比例不显著偏离0.5'}
    ...
    def analyze_all_data(self, csv_folder, txt_files, output_dir):
        ...
        # 创建可视化
        self.create_simple_visualization(combined_data, test_results, output_dir)
...
def main():
    ...
    # 创建分析器
    analyzer = SimpleBinomialTestAnalyzer()
