...
# -*- coding: utf-8 -*-
"""
category_heldout：解剖锚定声学 + 门控 EEG 残差（ResidualGatedAnatomicalPriority）。
默认 23 人（17 + 6，不含 hanglei 别名）。

输出：A:/.../individual/category_heldout_anat_gated_v1/
"""
...
from anatomical_acoustic_priority import ResidualGatedAnatomicalPriority, count_parameters
...
def train_gated_fold(fold_data, am_fm, device):
    ...
    model = ResidualGatedAnatomicalPriority(
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        for b in range(n_batches):
            ...
            opt.zero_grad()
            logits, _, delta, p_l, p_r, *_ = model(
        ...
        model.eval()
        with torch.no_grad():
            logits = model(X_val, mel_l_v, mel_r_v, s_l_v, s_r_v)
    ...
    return {
        "val_accuracy_anat_gated": float(accuracy_score(val_y, pred)),
        "val_balanced_accuracy_anat_gated": float(balanced_accuracy_score(val_y, pred)),
        "val_auc_anat_gated": float(roc_auc_score(val_y, prob))
...
def run_subject(subject_name, am_fm, pair_name_to_idx, type_map, out_dir, device):
    ...
    for fold_idx, val_idx in enumerate(folds, 1):
        ...
        m = train_gated_fold(fold_data, am_fm, device)
        row = {
        ...
        print(f"  Fold {fold_idx}: anat+gated BACC={m['val_balanced_accuracy_anat_gated']:.4f}")

    os.makedirs(out_dir, exist_ok=True)
    df = pd.DataFrame(fold_rows)
    csv_path = os.path.join(out_dir, f"{subject_name}_anat_gated_cv_results.csv")
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")

    b = df["val_balanced_accuracy_anat_gated"].mean()
    summary = (
        f"单人 category_heldout - {subject_name}\n"
        f"anatomical + residual_gated BACC: {b:.4f}\n"
    )
    with open(
        os.path.join(out_dir, f"{subject_name}_anat_gated_cv_summary.txt"), "w", encoding="utf-8"
    ) as f:
        f.write(summary)
    print(summary)
    return {"subject": subject_name, "bacc_anat_gated": float(b)}
...
def main():
    ...
    out_dir = os.path.join(OUTPUT_DIR, "individual", "category_heldout_anat_gated_v1")
    os.makedirs(out_dir, exist_ok=True)
    ...
    if rows:
        ...
        summary_name = f"n{len(rows)}_anat_gated_summary.csv"
        sdf.to_csv(os.path.join(out_dir, summary_name), index=False, encoding="utf-8-sig")
        print(f"\n=== {len(rows)} 人汇总 ({summary_name}) ===")
        print(sdf.to_string(index=False))
        print(f"group anat+gated BACC={sdf.bacc_anat_gated.mean():.4f}")
