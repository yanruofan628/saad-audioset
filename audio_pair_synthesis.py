...
def load_features_data(features_path):
    """加载已提取的特征数据"""
    pickle_path = os.path.join(features_path, "all_categories_features.pkl")
    if os.path.exists(pickle_path):
        try:
            with open(pickle_path, 'rb') as f:
                features_data = pickle.load(f)
            print(f"成功加载特征数据，包含 {len(features_data['categories'])} 个类别")
            return features_data
        except Exception as e:
            print(f"加载特征数据失败: {e}")
            return None
    else:
        print(f"特征数据文件不存在: {pickle_path}")
        return None
...
def load_audio_file(audio_path, target_sr=22050):
    """加载音频文件并统一采样率，确保为单声道"""
    try:
        # 使用librosa加载，mono=True确保输出单声道
...
def create_stereo_audio(left_audio, right_audio, output_path, sr=22050):
    """
    创建左右声道分离的双声道音频
    左声道：第一个音频，右声道：第二个音频
...
def get_available_audio_files(features_data, category_name, used_files):
    ...
    for file_info in category_data['features']:
        if (file_info['features'] is not None and 
...
def nearest_neighbor_matching(features_data, category_mapping, used_files, output_dir):
    ...
    for left_idx, (left_main_cat, left_sub_cat) in enumerate(all_subcategories):
        ...
        for right_idx, (right_main_cat, right_sub_cat) in enumerate(all_subcategories):
            ...
            # 获取左声道音频的特征
            left_features = left_file_info['features']['feature_matrix_normalized']
            left_optimized = aggregate_features(left_features, method='statistical')
            ...
            for right_file_info in audio_queues[(right_main_cat, right_sub_cat)]:
                # 获取右声道音频的特征
                right_features = right_file_info['features']['feature_matrix_normalized']
                right_optimized = aggregate_features(right_features, method='statistical')

                # 计算相似度
                similarity = cosine_similarity([left_optimized], [right_optimized])[0][0]
            ...
            if left_audio is not None and right_audio is not None:
                ...
                left_category_clean = clean_filename(left_category)
                right_category_clean = clean_filename(right_category)
                left_audio_id_clean = clean_filename(left_audio_id)
                right_audio_id_clean = clean_filename(right_audio_id)
                output_filename = f"nn_{left_category_clean}_{left_audio_id_clean}+{right_category_clean}_{right_audio_id_clean}.wav"
                output_path = os.path.join(output_dir, "nearest_neighbor_pairs", output_filename)
                os.makedirs(os.path.dirname(output_path), exist_ok=True)

                # 合成双声道音频
                success = create_stereo_audio(left_audio, right_audio, output_path, sr)
                if success:
...
def random_pairing(features_data, category_mapping, used_files, output_dir, nn_used_audios=None):
    ...
    总共15×15=225个配对
    如果某个类别可用音频不足15条，从最近邻匹配阶段移除的音频中随机补充
    """
    print(f"\n=== 开始随机配对阶段 ===")
    ...
    for left_idx, (left_main_cat, left_sub_cat) in enumerate(all_subcategories):
        ...
        for right_idx, (right_main_cat, right_sub_cat) in enumerate(all_subcategories):
            ...
            if left_audio is not None and right_audio is not None:
                ...
                left_audio_id_clean = clean_filename(left_audio_id)
                right_audio_id_clean = clean_filename(right_audio_id)
                output_filename = f"rn_{left_category_clean}_{left_audio_id_clean}+{right_category_clean}_{right_audio_id_clean}.wav"
...
def main(features_path, output_dir):
    ...
    # 加载特征数据
    features_data = load_features_data(features_path)
...
if __name__ == "__main__":
    ...
    output_dir = r"D:\D\research\audioset下载\音频配对合成"  # 输出目录
