...
# -*- coding: utf-8 -*-
"""
处理audio_pairs_2s文件夹中的音频文件
1. 从subcategory_pairs文件夹获取所有音频文件并打乱顺序
2. 将文件一分为三，分到三个文件夹（folder1、folder2、folder3）
...
def split_and_rename_audio_2s():
    # 源文件夹路径 - audio_pairs_2s/subcategory_pairs
    source_dir = Path(r"D:\D\research\audioset下载\audio_pairs_2s\subcategory_pairs")
    ...
    # 分配文件到三个文件夹
    # 如果有余数，前几个文件夹多分配一个文件
    folder1_end = part_size + (1 if remainder > 0 else 0)
    folder2_end = folder1_end + part_size + (1 if remainder > 1 else 0)
    
    folder1_files = all_files[:folder1_end]
    folder2_files = all_files[folder1_end:folder2_end]
    folder3_files = all_files[folder2_end:]
    
    print(f"将分配 {len(folder1_files)} 个文件到 folder1")
    print(f"将分配 {len(folder2_files)} 个文件到 folder2")
    print(f"将分配 {len(folder3_files)} 个文件到 folder3")
    
    # 创建目标文件夹
    output_base_dir = source_dir.parent / "分组音频"
    folder1_dir = output_base_dir / "folder1"
    folder2_dir = output_base_dir / "folder2"
    folder3_dir = output_base_dir / "folder3"
    
    folder1_dir.mkdir(parents=True, exist_ok=True)
    folder2_dir.mkdir(parents=True, exist_ok=True)
    folder3_dir.mkdir(parents=True, exist_ok=True)
    ...
    # 处理 folder3
    print(f"\n开始处理 folder3...")
    csv_data_folder3 = process_folder(folder3_files, folder3_dir)
    ...
    csv_path3 = folder3_dir / "file_mapping_folder3.csv"
    df3 = pd.DataFrame(csv_data_folder3)
    df3.to_csv(csv_path3, index=False, encoding='utf-8-sig')
    print(f"已生成CSV文件: {csv_path3}")
    
    # 验证结果
    print(f"\n验证结果:")
    print(f"folder1 中的文件数: {len(list(folder1_dir.glob('*.wav')))}")
    print(f"folder2 中的文件数: {len(list(folder2_dir.glob('*.wav')))}")
    print(f"folder3 中的文件数: {len(list(folder3_dir.glob('*.wav')))}")
    ...
    print(f"folder3: {len(folder3_files)} 个文件已重命名为 a001.wav 到 a{len(folder3_files):03d}.wav")
...
def preview_split():
    ...
    print(f"folder3 将包含: {len(folder3_files)} 个文件 (a001.wav 到 a{len(folder3_files):03d}.wav)")
    ...
    print(f"\nfolder3 前5个文件示例:")
    for i, file_path in enumerate(folder3_files[:5], 1):
        new_name = f"a{i:03d}.wav"
        print(f"  {new_name} <- {file_path.name}")
