...
2. 确保数据路径正确：
   - EEG数据：A:/standard_data_noica/{subject_name}
...
SUBJECTS_CONFIG = {
    'yanxingzhuo': {
        'processed_data_path': 'A:/standard_data_noica/yanxingzhuo',
        'mff_folders': [
    ...
    'jinxiaoyue': {
        'processed_data_path': 'A:/standard_data_noica/jinxiaoyue',
        'mff_folders': [
    ...
    'chenxianwei': {
        'processed_data_path': 'A:/standard_data_noica/chenxianwei',
        'mff_folders': [
    ...
    'yeziyuan': {
        'processed_data_path': 'A:/standard_data_noica/yeziyuan',
        'mff_folders': [
    ...
    'zhangzhiyao': {
        'processed_data_path': 'A:/standard_data_noica/zhangzhiyao',
        'mff_folders': [
    ...
    'haoxiang': {
        'processed_data_path': 'A:/standard_data_noica/haoxiang',
        'mff_folders': [
...
# 输出目录（改为多被试输出）
OUTPUT_DIR = 'A:/standard_data_noica/nme_fusion_results'
...
def build_trial_rating_left_right(trials, subject_ids, subject_names, all_subjects_ratings):
    """
    为每个 trial 构建左右类别的评分向量（6 维：左 fam/urg/lik + 右 fam/urg/lik）。

    参数:
        trials: trial 列表，每项含 'pair_name'
        subject_ids: (n_trials,) 被试 ID
        subject_names: 被试名列表，subject_names[subject_id] 为名
        all_subjects_ratings: {subject_name: {category: {'familiarity', 'urgency', 'liking'}}}

    返回:
        (n_trials, 6) float32，或 None（任一被试无评分时）
    """
    n = len(trials)
    out = []
    for i in range(n):
        trial = trials[i]
        sid = int(subject_ids[i]) if hasattr(subject_ids, '__getitem__') else subject_ids
        name = subject_names[sid]
        if name not in all_subjects_ratings:
            print(f"警告: 被试 {name} 无评分数据，无法构建 trial 级左右 rating")
            return None
        rating_dict = all_subjects_ratings[name]
        ...
        def vec(cat):
            if cat and cat in rating_dict:
                r = rating_dict[cat]
                return [float(r.get('familiarity', 0.0)), float(r.get('urgency', 0.0)), float(r.get('liking', 0.0))]
            return [0.0, 0.0, 0.0]
        left_vec = vec(left_cat)
        right_vec = vec(right_cat)
        out.append(left_vec + right_vec)
...
class NMEWithTrialRatingLogitsBiasClassifier(nn.Module):
    """NME + trial 级左右 rating 调节 logits（决策 bias）。输入：eeg, audio, rating_left_right (batch, 6), subject_ids。"""
    def __init__(self, eeg_channels, eeg_samples, audio_dim, n_subjects=5, hidden_dim=64, n_classes=2):
        super(NMEWithTrialRatingLogitsBiasClassifier, self).__init__()
        self.eeg_extractor = EEGNetFeatureExtractor(eeg_channels, eeg_samples)
        total_dim = 160 + audio_dim
        self.fixed_effects_fusion = nn.Sequential(
            nn.Linear(total_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.2),
            nn.Linear(hidden_dim, hidden_dim // 2), nn.ReLU(), nn.Dropout(0.2),
        )
        self.fixed_effects_classifier = nn.Linear(hidden_dim // 2, n_classes)
        self.random_intercepts = nn.ParameterDict({f'subject_{i}': nn.Parameter(torch.zeros(1)) for i in range(n_subjects)})
        self.random_slopes = nn.ParameterDict({f'subject_{i}': nn.Parameter(torch.zeros(hidden_dim // 2)) for i in range(n_subjects)})
        self.log_intercept_variance = nn.Parameter(torch.tensor(-2.0))
        self.log_slope_variance = nn.Parameter(torch.ones(hidden_dim // 2) * (-2.0))
        self.rating_encoder = nn.Sequential(nn.Linear(6, hidden_dim), nn.ReLU(), nn.Dropout(0.2), nn.Linear(hidden_dim, 1))
        self.rating_sensitivity = nn.ParameterDict({f'subject_{i}': nn.Parameter(torch.ones(1) * 0.1) for i in range(n_subjects)})

    def forward(self, eeg_data, audio_features, rating_left_right, subject_ids):
        batch_size = eeg_data.shape[0]
        if isinstance(subject_ids, torch.Tensor):
            subject_ids = subject_ids.tolist()
        elif isinstance(subject_ids, int):
        ...
        combined = torch.cat([eeg_feat, audio_features], dim=1)
        fixed_features = self.fixed_effects_fusion(combined)
        adjusted_features = []
        for i in range(batch_size):
            subject_key = f'subject_{subject_ids[i]}'
            h_i = fixed_features[i] + self.random_intercepts[subject_key] + self.random_slopes[subject_key] * fixed_features[i]
            adjusted_features.append(h_i)
        adjusted_features = torch.stack(adjusted_features)
        base_logits = self.fixed_effects_classifier(adjusted_features)
        bias_scalar = self.rating_encoder(rating_left_right)
        for i in range(batch_size):
            scale = self.rating_sensitivity[f'subject_{subject_ids[i]}']
            base_logits[i, 0] = base_logits[i, 0] + (scale * bias_scalar[i, 0]).squeeze()
        return base_logits
    ...
    def get_nme_regularization_loss(self):
        ...
        for subject_key in self.random_intercepts.keys():
            reg_loss += (self.random_intercepts[subject_key] ** 2) / (var_intercept + 1e-8)
            reg_loss += torch.sum(self.random_slopes[subject_key] ** 2) / (mean_var_slope + 1e-8)
        return reg_loss


class NMEWithTrialRatingAudioWeightClassifier(nn.Module):
    """NME + trial 级左右 rating 调节音频权重（gate）。输入：eeg, audio, rating_left_right (batch, 6), subject_ids。"""
    def __init__(self, eeg_channels, eeg_samples, audio_dim, n_subjects=5, hidden_dim=64, n_classes=2):
        super(NMEWithTrialRatingAudioWeightClassifier, self).__init__()
        self.eeg_extractor = EEGNetFeatureExtractor(eeg_channels, eeg_samples)
        total_dim = 160 + audio_dim
        self.fixed_effects_fusion = nn.Sequential(
            nn.Linear(total_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.2),
        ...
        self.log_intercept_variance = nn.Parameter(torch.tensor(-2.0))
        self.log_slope_variance = nn.Parameter(torch.ones(hidden_dim // 2) * (-2.0))
        self.rating_to_audio_gate = nn.Sequential(nn.Linear(6, hidden_dim), nn.ReLU(), nn.Dropout(0.2), nn.Linear(hidden_dim, audio_dim))
    ...
    def forward(self, eeg_data, audio_features, rating_left_right, subject_ids):
        ...
        if isinstance(subject_ids, torch.Tensor):
            ...
            subject_ids = [subject_ids] * batch_size
        eeg_feat = self.eeg_extractor(eeg_data)
        gate_logits = self.rating_to_audio_gate(rating_left_right)
        gates = torch.sigmoid(gate_logits)
        mod_audio = gates * audio_features
        combined = torch.cat([eeg_feat, mod_audio], dim=1)
        fixed_features = self.fixed_effects_fusion(combined)
        adjusted_features = []
        for i in range(batch_size):
            subject_key = f'subject_{subject_ids[i]}'
            ...
            adjusted_features.append(h_i)
        adjusted_features = torch.stack(adjusted_features)
        return self.fixed_effects_classifier(adjusted_features)
...
def train_and_evaluate_all_models(X_eeg_raw, X_audio_manual, y, subject_ids,
                                  ...
                                  rating_left_right=None, folds=None, shuffled_indices=None):
    """
    训练和评估三个模型：
    - nme_fusion: NME融合（无 rating）
    - nme_fusion_rating_logits: NME + trial 级左右 rating 调节 logits
    - nme_fusion_rating_audio: NME + trial 级左右 rating 调节音频权重
    ...
    参数:
        ...
        n_folds: 交叉验证折数
        seed: 随机种子
        consistent_trials: 包含pair_name信息的trial列表（用于划分fold）
        use_stratified_cv: 是否使用按被试分层的交叉验证（推荐True）
        rating_left_right: (n_trials, 6) trial 级左右评分向量；None 时仅训练 nme_fusion
        folds: 预先划分好的fold列表（如果提供，将直接使用，不再重新划分）
        shuffled_indices: 预先打乱的索引数组（与folds配合使用）
    """
    print("\n=== 训练和评估三个模型（NME / NME+rating_logits / NME+rating_audio）===")
    ...
    all_fold_results = {
        'nme_fusion': [],
        'nme_fusion_rating_logits': [],
        'nme_fusion_rating_audio': [],
    }
    ...
    for fold_idx, test_idx in enumerate(folds, 1):
        ...
        # 如果提供了 rating_left_right，添加两个 trial 级 rating 调制模型
        if rating_left_right is not None:
            models['nme_fusion_rating_logits'] = NMEWithTrialRatingLogitsBiasClassifier(
                eeg_channels, eeg_samples, audio_dim, n_subjects=n_subjects_actual
            )
            models['nme_fusion_rating_audio'] = NMEWithTrialRatingAudioWeightClassifier(
                eeg_channels, eeg_samples, audio_dim, n_subjects=n_subjects_actual
            )
            print(f"  创建 trial 级 rating 模型：nme_fusion_rating_logits, nme_fusion_rating_audio（rating_left_right 维度 6）")
        else:
            print(f"  未提供 rating_left_right，仅训练 nme_fusion")

        print(f"  创建 NME 模型：支持 {n_subjects_actual} 个被试（subject_0 到 subject_{n_subjects_actual - 1}）")
        ...
        for model_name, model in models.items():
            ...
            # Batch处理配置
            batch_size = 16 if model_name in ['nme_fusion', 'nme_fusion_rating_logits', 'nme_fusion_rating_audio'] else 32
            n_train = len(y_train)
            ...
            for epoch in range(n_epochs):
                ...
                if model_name in ['nme_fusion', 'nme_fusion_rating_logits', 'nme_fusion_rating_audio']:
                ...
                for batch_idx in range(n_batches):
                    ...
                    batch_subject_ids = subject_ids[train_idx[batch_train_idx]] if model_name in ['nme_fusion', 'nme_fusion_rating_logits', 'nme_fusion_rating_audio'] else None

                    # 获取batch对应的 rating_left_right（用于 trial 级 rating 模型）
                    batch_rating_left_right = None
                    if model_name in ['nme_fusion_rating_logits', 'nme_fusion_rating_audio'] and rating_left_right is not None:
                        batch_rating_left_right = torch.FloatTensor(
                            rating_left_right[train_idx[batch_train_idx]]
                        ).to(device)
                    ...
                    elif model_name in ['nme_fusion_rating_logits', 'nme_fusion_rating_audio']:
                        outputs = model(X_eeg_batch, X_audio_batch, batch_rating_left_right, batch_subject_ids)
                        classification_loss = criterion(outputs, y_batch)
                        regularization_loss = model.get_nme_regularization_loss()
                        loss = classification_loss + lambda_reg * regularization_loss
                ...
                if (epoch + 1) % 10 == 0 or epoch == n_epochs - 1:
                    ...
                    with torch.no_grad():
                        ...
                        val_correct = 0
                        val_total = 0
                        val_batch_size = 16 if model_name in ['nme_fusion', 'nme_fusion_rating_logits', 'nme_fusion_rating_audio'] else 32
                        n_val = len(y_val)
                        ...
                        for val_batch_idx in range(n_val_batches):
                            ...
                            if model_name == 'nme_fusion':
                                ...
                                val_batch_subject_ids = subject_ids[test_idx[val_batch_indices]]
                                val_batch_rating_left_right = torch.FloatTensor(
                                    rating_left_right[test_idx[val_batch_indices]]
                                ).to(device)
                                val_outputs = model(X_eeg_val_batch,
                                                    X_audio_val_batch,
                                                    val_batch_rating_left_right,
                        ...
                        # 计算验证集上的平均损失（NME 及 trial rating 模型）
                        if model_name in ['nme_fusion', 'nme_fusion_rating_logits', 'nme_fusion_rating_audio']:
            ...
            with torch.no_grad():
                ...
                # NME 及 trial rating 模型需要额外参数
                if model_name == 'nme_fusion':
                    val_subject_ids = subject_ids[test_idx]
                    val_outputs = model(X_eeg_val_batch, X_audio_val_batch, val_subject_ids)
                elif model_name in ['nme_fusion_rating_logits', 'nme_fusion_rating_audio']:
                    val_subject_ids = subject_ids[test_idx]
                    val_rating_left_right = torch.FloatTensor(rating_left_right[test_idx]).to(device)
                    val_outputs = model(X_eeg_val_batch,
                                        X_audio_val_batch,
                                        val_rating_left_right,
                                        val_subject_ids)
                else:
                    val_outputs = model(X_eeg_val_batch, X_audio_val_batch)
    ...
    for model_name, fold_results in all_fold_results.items():
        ...
        model_display_name = {
            ...
            'nme_fusion_rating_logits': 'NME+rating调节logits',
            'nme_fusion_rating_audio': 'NME+rating调节音频权重',
...
def main(filter_consistent=True):
    ...
    # ============ 加载评分并构建 trial 级左右 rating 向量 ============
    print(f"\n=== 加载评分并构建 trial 级左右 rating（6 维）===")

    all_subjects_ratings = load_all_subjects_ratings(RATING_EXCEL_PATH, subject_names)
    rating_left_right_array = build_trial_rating_left_right(
        all_merged_trials, all_merged_subject_ids, subject_names, all_subjects_ratings
    )
    if rating_left_right_array is not None:
        print(f"rating_left_right 数组形状: {rating_left_right_array.shape}")
    else:
        print(f"警告: 无法构建 trial 级 rating，将仅训练 nme_fusion")
    ...
    all_results = train_and_evaluate_all_models(
        ...
        rating_left_right=rating_left_right_array,
        folds=unified_folds,
    ...
    # 10. 三模型对比（nme_fusion / nme_fusion_rating_logits / nme_fusion_rating_audio）
    if 'nme_fusion' in all_results and (rating_left_right_array is not None):
        if 'nme_fusion_rating_logits' in all_results or 'nme_fusion_rating_audio' in all_results:
            print(f"\n=== 三模型对比（NME / NME+rating_logits / NME+rating_audio）===")
            nme_results = all_results['nme_fusion']
            comparison_path = os.path.join(OUTPUT_DIR, "nme_three_models_comparison.txt")
            with open(comparison_path, 'w', encoding='utf-8') as f:
                f.write("NME vs NME+rating_logits vs NME+rating_audio 对比\n")
                f.write("=" * 60 + "\n\n")
                f.write(f"NME（无rating）: 准确率={nme_results['mean_acc']:.4f}, 平衡准确率={nme_results['mean_balanced_acc']:.4f}, AUC={nme_results['mean_auc']:.4f}\n")
                for key in ['nme_fusion_rating_logits', 'nme_fusion_rating_audio']:
                    if key in all_results:
                        r = all_results[key]
                        f.write(f"{key}: 准确率={r['mean_acc']:.4f}, 平衡准确率={r['mean_balanced_acc']:.4f}, AUC={r['mean_auc']:.4f}\n")
            print(f"对比结果已保存: {comparison_path}")

    # 保存汇总信息
    summary_path = os.path.join(OUTPUT_DIR, f"fusion_classification_summary.txt")
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("多被试pair-wise交叉验证结果汇总（三模型）\n")
        f.write("=" * 60 + "\n")
        ...
        if rating_left_right_array is not None:
            f.write(f"rating_left_right_dim: 6\n")
        f.write(f"\n")
