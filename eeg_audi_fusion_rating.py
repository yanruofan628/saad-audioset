...
def parse_pair_name(pair_name):
    """
    解析音频对名称，提取左右音频的类别和ID
    
    参数:
        pair_name: 音频对名称，格式如 "类别1_id1_类别2_id2" 或 "类别1_id1_vs_类别2_id2"
    
    返回:
        (left_category, left_id, right_category, right_id, pair_name) 或 None
    """
    if not pair_name:
        return None
    
    # 尝试不同的分隔符
    separators = ['_vs_', '_VS_', '_']
    
    for sep in separators:
        if sep in pair_name:
            parts = pair_name.split(sep)
            if len(parts) >= 4:
                # 假设格式：类别1_id1_类别2_id2
                left_category = parts[0]
                left_id = parts[1]
                right_category = parts[2]
                right_id = parts[3]
                return (left_category, left_id, right_category, right_id, pair_name)
            elif len(parts) == 2:
                # 可能是简化的格式
                left_part = parts[0]
                right_part = parts[1]
                # 尝试进一步解析
                left_parts = left_part.split('_')
                right_parts = right_part.split('_')
                if len(left_parts) >= 2 and len(right_parts) >= 2:
                    left_category = left_parts[0]
                    left_id = left_parts[1]
                    right_category = right_parts[0]
                    right_id = right_parts[1]
                    return (left_category, left_id, right_category, right_id, pair_name)
...
def load_subject_rating_sheet(excel_path, sheet_name):
    ...
    参数:
        ...
        sheet_name: sheet名称（被试名称）
    
    返回:
        rating_dict: {category: {'familiarity': x, 'urgency': y, 'liking': z}}
    """
    ...
    try:
        ...
        print(f"成功加载 {sheet_name} 的评分数据: {len(rating_dict)} 个类别")
        return rating_dict
    except Exception as e:
        print(f"加载 {sheet_name} 的评分数据失败: {e}")
        import traceback
        traceback.print_exc()
        return None


def load_all_subjects_ratings(excel_path, subject_names):
    """
    加载所有被试的评分数据
    ...
    返回:
        all_ratings: {subject_name: rating_dict}
    """
    print(f"\n=== 加载所有被试的评分数据 ===")
    print(f"评分文件: {excel_path}")
    
    all_ratings = {}
    for subject_name in subject_names:
        rating_dict = load_subject_rating_sheet(excel_path, subject_name)
        if rating_dict:
            all_ratings[subject_name] = rating_dict
        else:
            print(f"警告: 无法加载 {subject_name} 的评分数据")
    
    print(f"成功加载 {len(all_ratings)}/{len(subject_names)} 个被试的评分数据")
    return all_ratings


def extract_all_categories_from_pair_names(pair_names):
    """
    从pair_names中提取所有唯一的类别
    
    参数:
        pair_names: 音频对名称列表
    
    返回:
        all_categories: 排序后的唯一类别列表
    """
    ...
    for pair_name in pair_names:
        ...
        if left_cat:
            all_categories_set.add(left_cat)
        if right_cat:
            all_categories_set.add(right_cat)
    
    all_categories = sorted(list(all_categories_set))
    print(f"\n从pair_names提取到 {len(all_categories)} 个唯一类别")
    return all_categories


def build_personal_rating_profile_vector(rating_dict, all_categories):
    """
    构建个人rating profile向量
    
    参数:
        rating_dict: {category: {'familiarity': x, 'urgency': y, 'liking': z}}
        all_categories: 所有类别的列表（确保向量维度一致）
    
    返回:
        rating_profile_vector: (n_categories * 3,) numpy数组
        例如：如果有10个类别 → 30维向量
        [cat1_fam, cat1_urg, cat1_lik, cat2_fam, cat2_urg, cat2_lik, ...]
    """
    ...
    for cat in all_categories:
        if cat in rating_dict:
            ratings = rating_dict[cat]
            rating_vector.extend([
                ratings.get('familiarity', 0.0),
                ratings.get('urgency', 0.0),
                ratings.get('liking', 0.0)
            ])
        else:
            # 如果某个类别没有rating，用0填充
            rating_vector.extend([0.0, 0.0, 0.0])
    
    rating_vector = np.array(rating_vector, dtype=np.float32)
    
    # 标准化（z-score）
    rating_mean = rating_vector.mean()
    rating_std = rating_vector.std()
    if rating_std > 0:
        rating_vector = (rating_vector - rating_mean) / rating_std
...
class NMEWithRatingProfilePriorClassifier(nn.Module):
    """
    NME框架 + Rating Profile向量作为先验
    
    架构设计：
    1. 固定效应：群体共享的EEG-音频融合模式
    2. 随机效应：每个被试的个性化调整参数（NME）
    3. Rating Profile先验：个人所有评分拼成的向量，调节注意力权重 / 音频特征
    
    当前实现支持三种rating调制方式（通过mode选择）：
    - 'global_gate' : 生成EEG vs Audio的全局权重（原始版本）
    - 'feature_gate': 生成对每个音频特征维度的gate（0~1），逐维调制audio特征
    - 'film'        : 生成对audio特征的FiLM参数gamma/beta，做逐维仿射变换
    """
    
    def __init__(self, eeg_channels, eeg_samples, audio_dim, 
                 rating_profile_dim, n_subjects=5, hidden_dim=64, n_classes=2,
                 mode: str = 'global_gate'):
        super(NMEWithRatingProfilePriorClassifier, self).__init__()
        
        assert mode in ['global_gate', 'feature_gate', 'film'], \
            f"Unsupported rating modulation mode: {mode}"
        self.mode = mode
        ...
        # ============ Rating Profile先验模块 ============
        # Rating Profile编码器：将rating profile向量编码为可学习的表示
        self.rating_profile_encoder = nn.Sequential(
            nn.Linear(rating_profile_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.ReLU(),
        ...
        # 注意力 / 调制网络输入：NME调整后的特征 + rating profile编码
        attention_input_dim = hidden_dim // 2 + hidden_dim // 2  # adjusted_features + rating_encoded
        
        # 方式1：global_gate - 生成EEG vs Audio的全局权重
        self.attention_modulation = nn.Sequential(
            nn.Linear(attention_input_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim // 2, 2),  # 输出2个权重：EEG权重和音频权重
        )
        
        # 方式2：feature_gate - 对每个audio特征维度生成一个gate（0~1）
        self.audio_feature_attention = nn.Sequential(
            nn.Linear(attention_input_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim // 2, audio_dim)  # 输出每个特征的gate
        )
        
        # 方式3：FiLM - 对audio特征生成gamma/beta（逐维仿射变换）
        self.film_generator = nn.Sequential(
            nn.Linear(attention_input_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim, 2 * audio_dim)  # 前半为gamma_raw，后半为beta_raw
        )
    ...
    def forward(self, eeg_data, audio_features, rating_profile, subject_ids, return_components=False):
        """
        前向传播
        ...
        参数:
            eeg_data: (batch, 1, n_channels, n_timepoints)
            audio_features: (batch, audio_dim)
            rating_profile: (batch, rating_profile_dim) - 个人rating profile向量
            subject_ids: (batch,) - 被试ID列表
            return_components: bool - 是否返回中间组件用于分析
        ...
        for i in range(batch_size):
            ...
            h_i = fixed_features[i]
            adjusted_h_i = h_i + intercept_i + slopes_i * h_i
            adjusted_features.append(adjusted_h_i)
        ...
        # ============ Rating Profile先验调制 ============
        # 1. 编码rating profile
        rating_encoded = self.rating_profile_encoder(rating_profile)  # (batch, hidden_dim//2)
        
        # 2. 组合NME调整后的特征与rating编码，作为调制网络的输入
        attention_input = torch.cat([adjusted_features, rating_encoded], dim=1)  # (batch, hidden_dim)
        
        # 3. 根据不同mode进行调制
        if self.mode == 'global_gate':
            # 3.1 方式1：生成EEG vs Audio的全局权重
            attention_logits = self.attention_modulation(attention_input)  # (batch, 2)
            attention_weights = F.softmax(attention_logits, dim=1)  # (batch, 2)
            
            weighted_eeg = attention_weights[:, 0:1] * eeg_feat  # (batch, 160)
            weighted_audio = attention_weights[:, 1:2] * audio_features  # (batch, audio_dim)
            weighted_combined = torch.cat([weighted_eeg, weighted_audio], dim=1)  # (batch, 160+audio_dim)
        
        elif self.mode == 'feature_gate':
            # 3.2 方式2：对每个audio特征维度生成gate（0~1），逐维调制audio
            audio_logits = self.audio_feature_attention(attention_input)  # (batch, audio_dim)
            audio_gates = torch.sigmoid(audio_logits)  # (batch, audio_dim), 每维0~1
            mod_audio = audio_gates * audio_features  # (batch, audio_dim)
            
            # EEG保持原始特征，只对audio做被试相关重加权
            weighted_combined = torch.cat([eeg_feat, mod_audio], dim=1)  # (batch, 160+audio_dim)
        
        elif self.mode == 'film':
            # 3.3 方式3：FiLM，对audio特征做逐维仿射变换
            film_params = self.film_generator(attention_input)  # (batch, 2*audio_dim)
            gamma_raw, beta_raw = torch.split(film_params, self.audio_dim, dim=1)  # 各自(batch, audio_dim)
            
            # 将gamma限制在接近1的范围内，避免数值发散；beta限制在较小范围
            gamma = 1.0 + 0.1 * torch.tanh(gamma_raw)  # (batch, audio_dim)
            beta = 0.1 * torch.tanh(beta_raw)         # (batch, audio_dim)
            
            mod_audio = gamma * audio_features + beta  # (batch, audio_dim)
            weighted_combined = torch.cat([eeg_feat, mod_audio], dim=1)  # (batch, 160+audio_dim)
        
        else:
            raise ValueError(f"Unsupported rating modulation mode: {self.mode}")
        
        # 4. 再次通过固定效应融合
        final_features = self.fixed_effects_fusion(weighted_combined)  # (batch, hidden_dim//2)
        
        # 5. 最终分类
        logits = self.fixed_effects_classifier(final_features)
    ...
    def get_nme_regularization_loss(self):
        ...
        log_var_intercept = torch.clamp(self.log_intercept_variance, min=-10, max=10)
        var_intercept = torch.exp(log_var_intercept)
        
        log_var_slope = torch.clamp(self.log_slope_variance, min=-10, max=10)
        var_slope = torch.exp(log_var_slope)
        mean_var_slope = torch.mean(var_slope)
...
def train_and_evaluate_all_models(X_eeg_raw, X_audio_manual, y, subject_ids,
                                  ...
                                  rating_profiles=None, folds=None, shuffled_indices=None):
    """
    训练和评估所有端到端融合模型：
    - simple: 简单拼接融合（端到端）
    - attention: 注意力融合（端到端）
    ...
    参数:
        ...
        folds: 预先划分好的fold列表（如果提供，将直接使用，不再重新划分）
        shuffled_indices: 预先打乱的索引数组（与folds配合使用）
    """
    print("\n=== 训练和评估所有端到端融合模型 ===")
    ...
    # 选择交叉验证方法
    # 优先：如果提供了预先划分好的folds，直接使用
    if folds is not None and shuffled_indices is not None:
        print(f"\n使用预先划分好的交叉验证fold（确保所有模型使用相同的划分）")
        # 验证folds和shuffled_indices的有效性
        if len(folds) != n_folds:
            raise ValueError(f"提供的folds数量({len(folds)})与n_folds({n_folds})不匹配")
    # 其次：如果提供了consistent_trials，使用build_pair_consistent_folds确保AB/BA在同一fold
    elif consistent_trials is not None and len(consistent_trials) == len(y):
    ...
    all_fold_results = {
        ...
        'nme_fusion': [],
        # 带rating先验的三种NME变体
        'nme_fusion_with_rating': [],              # 方式1：global_gate（EEG vs Audio整体权重）
        'nme_fusion_with_rating_feature': [],      # 方式2：feature_gate（逐维audio特征gate）
        'nme_fusion_with_rating_film': []          # 方式3：film（对audio做FiLM仿射调制）
    }
    ...
    for fold_idx, test_idx in enumerate(folds, 1):
        ...
        # 如果提供了rating_profiles，添加三种带rating调制的NME模型
        rating_models = []
        if rating_profiles is not None:
            rating_profile_dim = rating_profiles.shape[1] if len(rating_profiles.shape) > 1 else len(rating_profiles[0])
            
            # 方式1：global_gate（原始结构，调制EEG vs Audio整体权重）
            models['nme_fusion_with_rating'] = NMEWithRatingProfilePriorClassifier(
                eeg_channels, eeg_samples, audio_dim,
                rating_profile_dim=rating_profile_dim,
                n_subjects=n_subjects_actual,
                mode='global_gate'
            )
            rating_models.append('nme_fusion_with_rating')
            
            # 方式2：feature_gate（逐维audio特征gate）
            models['nme_fusion_with_rating_feature'] = NMEWithRatingProfilePriorClassifier(
                eeg_channels, eeg_samples, audio_dim,
                rating_profile_dim=rating_profile_dim,
                n_subjects=n_subjects_actual,
                mode='feature_gate'
            )
            rating_models.append('nme_fusion_with_rating_feature')
            
            # 方式3：film（FiLM仿射调制audio特征）
            models['nme_fusion_with_rating_film'] = NMEWithRatingProfilePriorClassifier(
                eeg_channels, eeg_samples, audio_dim,
                rating_profile_dim=rating_profile_dim,
                n_subjects=n_subjects_actual,
                mode='film'
            )
            rating_models.append('nme_fusion_with_rating_film')
            
            print(f"  创建带Rating Profile的NME模型：rating_profile维度={rating_profile_dim}，模式={rating_models}")
        ...
        for model_name, model in models.items():
            ...
            # Batch处理配置
            batch_size = 16 if model_name in ['sync_fusion', 'nme_fusion',
                                              'nme_fusion_with_rating',
                                              'nme_fusion_with_rating_feature',
                                              'nme_fusion_with_rating_film'] else 32  # 同步性模型和NME模型用更小的batch
            n_train = len(y_train)
            ...
            for epoch in range(n_epochs):
                ...
                if model_name in ['nme_fusion',
                                  'nme_fusion_with_rating',
                                  'nme_fusion_with_rating_feature',
                                  'nme_fusion_with_rating_film']:
                ...
                for batch_idx in range(n_batches):
                    ...
                    batch_subject_ids = subject_ids[train_idx[batch_train_idx]] if model_name in ['nme_fusion',
                                                                                                   'nme_fusion_with_rating',
                                                                                                   'nme_fusion_with_rating_feature',
                                                                                                   'nme_fusion_with_rating_film'] else None
                    
                    # 获取batch对应的rating_profiles（用于带rating的NME模型）
                    batch_rating_profiles = None
                    if model_name in ['nme_fusion_with_rating',
                                      'nme_fusion_with_rating_feature',
                                      'nme_fusion_with_rating_film'] and rating_profiles is not None:
                        batch_rating_profiles = torch.FloatTensor(
                            rating_profiles[train_idx[batch_train_idx]]
                        ).to(device)
                    ...
                    elif model_name in ['nme_fusion_with_rating',
                                        'nme_fusion_with_rating_feature',
                                        'nme_fusion_with_rating_film']:
                        # 带rating的NME模型需要subject_ids和rating_profiles参数
                        outputs = model(X_eeg_batch, X_audio_batch, batch_rating_profiles, batch_subject_ids)
                        classification_loss = criterion(outputs, y_batch)
                        regularization_loss = model.get_nme_regularization_loss()
                        loss = classification_loss + lambda_reg * regularization_loss
                ...
                if (epoch + 1) % 10 == 0 or epoch == n_epochs - 1:
                    model.eval()
                    with torch.no_grad():
                        # 验证也使用batch处理（特别是同步性模型和NME模型）
                        val_correct = 0
                        val_total = 0
                        val_batch_size = 16 if model_name in ['sync_fusion', 'nme_fusion', 'nme_fusion_with_rating'] else 32
                        n_val = len(y_val)
                        ...
                        for val_batch_idx in range(n_val_batches):
                            ...
                            if model_name == 'sync_fusion':
                                ...
                                val_regularization_loss += val_batch_regularization_loss.item()
                                                ...
                                                'nme_fusion_with_rating_film']:
                                val_batch_subject_ids = subject_ids[test_idx[val_batch_indices]]
                                val_batch_rating_profiles = torch.FloatTensor(
                                    rating_profiles[test_idx[val_batch_indices]]
                                ).to(device)
                                val_outputs = model(X_eeg_val_batch,
                                                    X_audio_val_batch,
                                                    val_batch_rating_profiles,
                                                    val_batch_subject_ids)
            ...
            with torch.no_grad():
                ...
                # 同步性模型和NME模型需要额外参数
                if model_name == 'sync_fusion':
                    # 同步性模型需要trial索引（用于从缓存中取同步性特征）
                    val_outputs = model(X_eeg_val_batch, X_audio_val_batch, test_idx)
                elif model_name == 'nme_fusion':
                    # 标准NME模型需要subject_ids
                    val_subject_ids = subject_ids[test_idx]
                    val_outputs = model(X_eeg_val_batch, X_audio_val_batch, val_subject_ids)
                elif model_name in ['nme_fusion_with_rating',
                                    'nme_fusion_with_rating_feature',
                                    'nme_fusion_with_rating_film']:
                    # 带rating的NME模型需要rating_profile和subject_ids
                    val_subject_ids = subject_ids[test_idx]
                    val_rating_profiles = None
                    if rating_profiles is not None:
                        val_rating_profiles = torch.FloatTensor(rating_profiles[test_idx]).to(device)
                    val_outputs = model(X_eeg_val_batch,
                                        X_audio_val_batch,
                                        val_rating_profiles,
                                        val_subject_ids)
...
def main(filter_consistent=True):
    ...
    if all_audio_features is None:
        ...
        left_audio_features, right_audio_features, feature_mapping = extract_audio_features(all_audio_trials,
                                                                                            project_root)
    ...
    # ============ 合并所有被试的数据，复用已提取的音频特征 ============
    print(f"\n=== 合并所有被试的数据（复用已提取的音频特征） ===")
    
    # 合并所有被试的EEG数据、标签和trials
    all_merged_eeg = []
    all_merged_labels = []
    all_merged_subject_ids = []
    all_merged_trials = []
    
    for subj_idx, subject_name in enumerate(subject_names):
        if subject_name in subject_consistent_trials:
            consistent_trials = subject_consistent_trials[subject_name]
            eeg_data = subject_eeg_data[subject_name]
            labels = subject_labels[subject_name]
            
            for i, trial in enumerate(consistent_trials):
                if i < len(eeg_data) and i < len(labels):
                    all_merged_eeg.append(eeg_data[i])
                    all_merged_labels.append(labels[i])
                    all_merged_subject_ids.append(subj_idx)
                    all_merged_trials.append(trial)
    
    all_merged_eeg = np.array(all_merged_eeg)
    all_merged_labels = np.array(all_merged_labels)
    all_merged_subject_ids = np.array(all_merged_subject_ids)
    
    print(f"合并后的数据:")
    print(f"  EEG数据形状: {all_merged_eeg.shape}")
    print(f"  标签数量: {len(all_merged_labels)}")
    print(f"  被试ID数量: {len(all_merged_subject_ids)}")
    
    # 复用已提取的音频特征，为每个trial获取对应的音频特征
    print(f"\n复用已提取的音频特征...")
    all_merged_audio = []
    for trial in all_merged_trials:
        pair_name = trial.get('pair_name') or trial.get('original_name')
        if pair_name and pair_name in pair_name_to_idx:
            audio_idx = pair_name_to_idx[pair_name]
            if audio_idx < len(all_audio_features):
                all_merged_audio.append(all_audio_features[audio_idx])
            else:
                print(f"警告: 音频索引 {audio_idx} 超出范围，使用第一个特征")
                all_merged_audio.append(all_audio_features[0])
        else:
            print(f"警告: 找不到trial的pair_name，使用第一个特征")
            all_merged_audio.append(all_audio_features[0])
    
    all_merged_audio = np.array(all_merged_audio)
    print(f"合并后的音频特征形状: {all_merged_audio.shape}")
    
    # ============ 加载和构建Rating Profile ============
    print(f"\n=== 加载和构建Rating Profile ===")
    
    # 1. 提取所有类别
    all_categories = extract_all_categories_from_pair_names(pair_names)
    print(f"提取到 {len(all_categories)} 个唯一类别")
    
    # 2. 加载所有被试的评分数据
    all_subjects_ratings = load_all_subjects_ratings(RATING_EXCEL_PATH, subject_names)
    
    # 3. 为每个被试构建rating profile向量
    subject_rating_profiles = {}
    for subject_name in subject_names:
        if subject_name in all_subjects_ratings:
            rating_dict = all_subjects_ratings[subject_name]
            rating_profile = build_personal_rating_profile_vector(rating_dict, all_categories)
            if rating_profile is not None:
                subject_rating_profiles[subject_name] = rating_profile
                print(f"  {subject_name}: rating profile维度={len(rating_profile)}")
            else:
                print(f"  警告: {subject_name} 的rating profile构建失败")
        else:
            print(f"  警告: {subject_name} 没有评分数据")
    
    # 4. 为每个trial构建对应的rating profile向量
    rating_profiles_array = []
    for i, trial in enumerate(all_merged_trials):
        subject_id = all_merged_subject_ids[i]
        subject_name = subject_names[subject_id]
        
        if subject_name in subject_rating_profiles:
            rating_profiles_array.append(subject_rating_profiles[subject_name])
        else:
            # 如果没有rating数据，使用零向量
            if len(subject_rating_profiles) > 0:
                # 使用第一个有效的rating profile的维度
                first_profile = list(subject_rating_profiles.values())[0]
                rating_profiles_array.append(np.zeros_like(first_profile))
            else:
                print(f"错误: 没有有效的rating profile数据")
                rating_profiles_array = None
                break
    
    if rating_profiles_array is not None:
        rating_profiles_array = np.array(rating_profiles_array)
        print(f"Rating profiles数组形状: {rating_profiles_array.shape}")
    else:
        print(f"警告: 无法构建rating profiles数组，将不使用rating调制")
        rating_profiles_array = None
    
    # ============ 统一划分交叉验证fold ============
    print(f"\n=== 统一划分交叉验证fold（确保所有模型使用相同的划分） ===")
    
    # 使用build_pair_consistent_folds确保AB/BA在同一fold，并使用固定seed保证可重复性
    unified_folds, unified_shuffled_indices = build_pair_consistent_folds(
        all_merged_trials, n_folds=N_FOLDS, seed=CV_SEED
    )
    
    print(f"已统一划分 {len(unified_folds)} 个fold")
    for fold_idx, fold in enumerate(unified_folds, 1):
        print(f"  Fold {fold_idx}: {len(fold)} 个样本")
    
    # ============ 训练和评估模型 ============
    print(f"\n=== 训练和评估所有模型 ===")
    
    # 获取EEG维度
    eeg_channels = all_merged_eeg.shape[1]
    eeg_samples = all_merged_eeg.shape[2]
    
    # 调用train_and_evaluate_all_models，传入预先划分好的folds
    all_results = train_and_evaluate_all_models(
        X_eeg_raw=all_merged_eeg,
        X_audio_manual=all_merged_audio,
        y=all_merged_labels,
        subject_ids=all_merged_subject_ids,
        eeg_channels=eeg_channels,
        eeg_samples=eeg_samples,
        n_folds=N_FOLDS,
        seed=CV_SEED,
        consistent_trials=all_merged_trials,
        use_stratified_cv=True,
        rating_profiles=rating_profiles_array,
        folds=unified_folds,
        shuffled_indices=unified_shuffled_indices
    )
    ...
    for model_name, result_dict in all_results.items():
        ...
        results_path = os.path.join(OUTPUT_DIR, f"fusion_classification_results_{model_name_safe}.csv")
        results_df.to_csv(results_path, index=False, encoding='utf-8-sig')
        print(f"\n{model_name}模型结果已保存: {results_path}")
    
    # 10. 对比NME和NME+Rating的结果
    if 'nme_fusion' in all_results and 'nme_fusion_with_rating' in all_results:
        print(f"\n=== NME vs NME+Rating 对比分析 ===")
        
        nme_results = all_results['nme_fusion']
        nme_rating_results = all_results['nme_fusion_with_rating']
        
        comparison_path = os.path.join(OUTPUT_DIR, "nme_vs_nme_rating_comparison.txt")
        with open(comparison_path, 'w', encoding='utf-8') as f:
            f.write("NME vs NME+Rating Profile 对比分析\n")
            f.write("=" * 60 + "\n\n")
            
            f.write("原始NME模型结果:\n")
            f.write(f"  平均验证准确率: {nme_results['mean_acc']:.4f} ± {nme_results['results_df']['val_accuracy'].std():.4f}\n")
            f.write(f"  平均平衡准确率: {nme_results['mean_balanced_acc']:.4f} ± {nme_results['results_df']['val_balanced_accuracy'].std():.4f}\n")
            f.write(f"  平均AUC: {nme_results['mean_auc']:.4f} ± {nme_results['results_df']['val_auc'].std():.4f}\n\n")
            
            f.write("NME+Rating Profile模型结果:\n")
            f.write(f"  平均验证准确率: {nme_rating_results['mean_acc']:.4f} ± {nme_rating_results['results_df']['val_accuracy'].std():.4f}\n")
            f.write(f"  平均平衡准确率: {nme_rating_results['mean_balanced_acc']:.4f} ± {nme_rating_results['results_df']['val_balanced_accuracy'].std():.4f}\n")
            f.write(f"  平均AUC: {nme_rating_results['mean_auc']:.4f} ± {nme_rating_results['results_df']['val_auc'].std():.4f}\n\n")
            
            f.write("性能提升:\n")
            acc_improvement = nme_rating_results['mean_acc'] - nme_results['mean_acc']
            balanced_acc_improvement = nme_rating_results['mean_balanced_acc'] - nme_results['mean_balanced_acc']
            auc_improvement = nme_rating_results['mean_auc'] - nme_results['mean_auc']
            
            f.write(f"  准确率提升: {acc_improvement:+.4f} ({acc_improvement/nme_results['mean_acc']*100:+.2f}%)\n")
            f.write(f"  平衡准确率提升: {balanced_acc_improvement:+.4f} ({balanced_acc_improvement/nme_results['mean_balanced_acc']*100:+.2f}%)\n")
            f.write(f"  AUC提升: {auc_improvement:+.4f} ({auc_improvement/nme_results['mean_auc']*100:+.2f}%)\n\n")
            
            f.write("各折详细对比:\n")
            for fold_idx in range(len(nme_results['results_df'])):
                nme_fold = nme_results['results_df'].iloc[fold_idx]
                nme_rating_fold = nme_rating_results['results_df'].iloc[fold_idx]
                f.write(f"  Fold {fold_idx + 1}:\n")
                f.write(f"    NME: 准确率={nme_fold['val_accuracy']:.4f}, 平衡准确率={nme_fold['val_balanced_accuracy']:.4f}, AUC={nme_fold['val_auc']:.4f}\n")
                f.write(f"    NME+Rating: 准确率={nme_rating_fold['val_accuracy']:.4f}, 平衡准确率={nme_rating_fold['val_balanced_accuracy']:.4f}, AUC={nme_rating_fold['val_auc']:.4f}\n")
                f.write(f"    提升: 准确率={nme_rating_fold['val_accuracy']-nme_fold['val_accuracy']:+.4f}, "
                       f"平衡准确率={nme_rating_fold['val_balanced_accuracy']-nme_fold['val_balanced_accuracy']:+.4f}, "
                       f"AUC={nme_rating_fold['val_auc']-nme_fold['val_auc']:+.4f}\n\n")
        
        print(f"对比分析结果已保存: {comparison_path}")
        
        # 打印简要对比
        print(f"\n性能对比:")
        print(f"  准确率: {nme_results['mean_acc']:.4f} → {nme_rating_results['mean_acc']:.4f} ({acc_improvement:+.4f})")
        print(f"  平衡准确率: {nme_results['mean_balanced_acc']:.4f} → {nme_rating_results['mean_balanced_acc']:.4f} ({balanced_acc_improvement:+.4f})")
        print(f"  AUC: {nme_results['mean_auc']:.4f} → {nme_rating_results['mean_auc']:.4f} ({auc_improvement:+.4f})")

    # 保存汇总信息
    summary_path = os.path.join(OUTPUT_DIR, f"fusion_classification_summary.txt")
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("多被试pair-wise交叉验证结果汇总\n")
        f.write("=" * 60 + "\n")
        f.write(f"训练样本数: {len(all_merged_eeg)}\n")
        f.write(f"eeg_channels: {eeg_channels}\n")
        f.write(f"eeg_samples: {eeg_samples}\n")
        f.write(f"audio_feature_dim: {all_merged_audio.shape[1]}\n")
        if rating_profiles_array is not None:
            f.write(f"rating_profile_dim: {rating_profiles_array.shape[1]}\n")
        f.write(f"\n")
        ...
        for model_name, result_dict in all_results.items():
            model_display_name = {
                ...
                'sync_fusion': '同步性融合',
                'nme_fusion': 'NME融合',
                'nme_fusion_with_rating': 'NME+Rating Profile融合'
            }.get(model_name, model_name)

            f.write(f"{model_display_name}:\n")
            f.write(f"  测试准确率: {result_dict['mean_acc']:.4f}\n")
            f.write(f"  测试平衡准确率: {result_dict['mean_balanced_acc']:.4f}\n")
            f.write(f"  测试AUC: {result_dict['mean_auc']:.4f}\n\n")
