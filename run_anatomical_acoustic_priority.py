...
# -*- coding: utf-8 -*-
"""
category_heldout：解剖锚定纯声学优先级（AnatomicalAcousticPriority）。
Mel 前端复用 am_fm_v2 缓存；反对称 dS = q_R − q_L。

输出：A:/.../individual/category_heldout_anat_prio_v1/
"""
...
from anatomical_acoustic_priority import AnatomicalAcousticPriority, count_parameters
from linear_regression_loudness_models import load_stereo_2s, parse_pair_name
from prime_ablation_ctrl import SUBJECTS_23, fold_seed, seed_everything, epoch_perm, seed_batch
...
if _env_subjects:
    SUBJECTS = [s.strip() for s in _env_subjects.split(",") if s.strip()]
elif os.environ.get("N23", "").strip() == "1":
    SUBJECTS = list(SUBJECTS_23)
else:
    SUBJECTS = list(SUBJECTS_DEFAULT_17)
...
# smoke: SUBJECTS=yanxingzhuo MAX_FOLDS=1
...
def train_anat_fold(fold_data, am_fm, device, fold_idx: int):
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        for b in range(n_batches):
            ...
            loss = crit(logits, yb)
            if PRIO_W > 0:
                loss = loss + PRIO_W * content_priority_margin_loss(p_l, p_r, yb)
            loss.backward()
            opt.step()
    ...
    return {
        "val_accuracy_anat_prio": float(accuracy_score(val_y, pred)),
        "val_balanced_accuracy_anat_prio": float(balanced_accuracy_score(val_y, pred)),
        "val_auc_anat_prio": float(roc_auc_score(val_y, prob))
...
def run_subject(subject_name, am_fm, pair_name_to_idx, type_map, out_dir, device):
    ...
    for fold_idx, val_idx in enumerate(folds, 1):
        ...
        m = train_anat_fold(fold_data, am_fm, device, fold_idx)
        row = {
        ...
        print(f"  Fold {fold_idx}: anat_prio BACC={m['val_balanced_accuracy_anat_prio']:.4f}")

    os.makedirs(out_dir, exist_ok=True)
    df = pd.DataFrame(fold_rows)
    csv_path = os.path.join(out_dir, f"{subject_name}_anat_prio_cv_results.csv")
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")

    b = df["val_balanced_accuracy_anat_prio"].mean()
    summary = (
        f"单人 category_heldout - {subject_name}\n"
        f"anatomical_acoustic_priority BACC: {b:.4f}\n"
    )
    with open(
        os.path.join(out_dir, f"{subject_name}_anat_prio_cv_summary.txt"), "w", encoding="utf-8"
    ) as f:
        f.write(summary)
    print(summary)
    return {"subject": subject_name, "bacc_anat_prio": float(b)}
...
def main():
    ...
    n_params = count_parameters(AnatomicalAcousticPriority())
    print("device:", device)
    ...
    print(f"AnatomicalAcousticPriority params: {n_params}")
    if MAX_FOLDS is not None:
        print(f"MAX_FOLDS={MAX_FOLDS} (smoke mode)")
    ...
    out_dir = os.path.join(OUTPUT_DIR, "individual", f"category_heldout_anat_prio_{RUN_TAG}")
    os.makedirs(out_dir, exist_ok=True)
