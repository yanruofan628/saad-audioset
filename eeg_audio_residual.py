...
使用方法：
1. 修改脚本底部 RUN_MODE：'pooled' 混训所有人；'individual' 每人单独训练
2. 修改 MODELS_TO_TRAIN 选择要训练的模型（默认 audio_only + residual）
3. individual 模式选被试：SUBJECTS_TO_RUN = ['hanglei', 'yanxingzhuo']，或 SUBJECT_NAME = 'hanglei'，或都为 None 跑全部
4. 修改 FILTER_CONSISTENT 选择是否筛选一致 trials
3. 确保数据路径正确：
   - EEG数据：D:/standard_noica_nointer/{subject_name}
   - 音频映射：audio_pairs_2s\\分组音频\\file_mapping_folder{1,2,3}.csv
   - 实验音频：csv 中 original_path（2s 立体声 subcategory_pairs）或 分组音频\\folder{X}\\a001.wav
3. 运行脚本：python eeg_audio_fusion_classify_hanglei.py

输出：
- pooled 模式: fusion_classification_cross_validation_results.csv / summary.txt
- individual 模式: individual/{subject}_residual_cv_results.csv / summary.txt, all_subjects_summary.csv
"""
import os
import sys
import json
...
from linear_regression_selected_features import (
    ...
    build_consistent_folds, load_wav2vec_feature_zscore
)
from linear_regression_loudness_models import (
    parse_stereo_pair_name, load_mono_5s, load_stereo_2s, DEFAULT_AUDIO_BASE_DIR,
...
from eeg_audio_residual_attention import (
    ResidualEEGFusionClassifier,
    ResidualEEGFusionCrossAttnSingleQuery,
    ResidualEEGFusionCrossAttnDualQuery,
)
...
# 数据配置
SAMPLING_RATE = 250
EEG_DATA_BASE_DIR = 'D:/standard_noica_nointer'

# 五个被试的完整配置
SUBJECTS_CONFIG = {
    'yanxingzhuo': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/yanxingzhuo',
        'mff_folders': [
    ...
    'hanglei': {
        # 兼容旧名；真实 EEG 目录为 zhanghanglei（见下方独立条目）
        'processed_data_path': 'A:/standard_data_interp_ica_99/zhanghanglei',
        'mff_folders': [
    ...
    'zhanghanglei': {
        ...
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': {'folder1': -1, 'folder2': -1}
    },
    'jichengzhi': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/jichengzhi',
        'mff_folders': [
            'A:/jichengzhi_1_20260121_095110.mff',
            'A:/jichengzhi_2_20260121_101353.mff',
            'A:/jichengzhi_3_20260121_103616.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-2-{session}',
        'missing_trials': None
    },
    'liuzehao': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/liuzehao',
        'mff_folders': [
            'A:/liuzehao_1_20260121_081425.mff',
            'A:/liuzehao_2_20260121_083817.mff',
            'A:/liuzehao_3_20260121_090121.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'zengdexin': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/zengdexin',
        'mff_folders': [
            'A:/zengdexin_1_20260119_103206.mff',
            'A:/zengdexin_2_20260119_105824.mff',
            'A:/zengdexin_3_20260119_112652.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'zhangyajie': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/zhangyajie',
        'mff_folders': [
            'A:/zhangyajie_1_20260128_080735.mff',
            'A:/zhangyajie_2_20260128_083459.mff',
            'A:/zhangyajie_3_20260128_090235.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'zhangyufei': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/zhangyufei',
        'mff_folders': [
            'A:/zhangyufei0106_1_20260106_054423.mff',
            'A:/zhangyufei0106_2_20260106_060959.mff',
            'A:/zhangyufei0106_3_20260106_063511.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'jinxiaoyue': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/jinxiaoyue',
        'mff_folders': [
    ...
    'chenxianwei': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/chenxianwei',
        'mff_folders': [
    ...
    'yeziyuan': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/yeziyuan',
        'mff_folders': [
    ...
    'zhangzhiyao': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/zhangzhiyao',
        'mff_folders': [
    ...
    'haoxiang': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/haoxiang',
        'mff_folders': [
    ...
    'hehaohuai': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/hehaohuai',
        'mff_folders': [
            'A:/hehaohuai0113_1_20260113_081454.mff',
            'A:/hehaohuai0113_2_20260113_084004.mff',
            'A:/hehaohuai0113_3_20260113_090254.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'qiuhaiyun': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/qiuhaiyun',
        'mff_folders': [
            'A:/qiuhaiyun_1_20260113_102231.mff',
            'A:/qiuhaiyun_2_20260113_105243.mff',
            'A:/qiuhaiyun_3_20260113_111816.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-2-{session}',
        'missing_trials': None
    },
    'zhouyu': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/zhouyu',
        'mff_folders': [
            'A:/zhouyu_1_20260114_052824.mff',
            'A:/zhouyu_2_20260114_055651.mff',
            'A:/zhouyu_3_20260114_062150.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'honghaokai': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/honghaokai',
        'mff_folders': [
            'A:/honghaokai_1_20260114_093709.mff',
            'A:/honghaokai_2_20260114_100206.mff',
            'A:/honghaokuai_3_20260114_103204.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-2-{session}',
        'missing_trials': None
    },
    ...
    'caolulu': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/caolulu',
        'mff_folders': [
            'A:/caolulu_1_20260115_082202.mff',
            'A:/caolulu_2_20260115_085517.mff',
            'A:/caolulu_3_20260115_092255.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'yanyinsong': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/yanyinsong',
        'mff_folders': [
            'A:/yanyinsong_1_20260115_101153.mff',
            'A:/yanyinsong_2_20260115_103940.mff',
            'A:/yanyinsong_3_20260115_110419.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-2-{session}',
        'missing_trials': None
    },
    'huangxiaohang': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/huangxiaohang',
        'mff_folders': [
            'A:/huangxiaohang_1_20260120_053343.mff',
            'A:/huangxiaohang_2_20260120_060155.mff',
            'A:/huangxiaohang_3_20260120_062633.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'xufan': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/xufan',
        'mff_folders': [
            'A:/xufan0120_1_20260120_102003.mff',
            'A:/xufan_2_20260120_104658.mff',
            'A:/xufan_3_20260120_111310.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-2-{session}',
        'missing_trials': None
    },
    'qiusiqi': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/qiusiqi',
        'mff_folders': [
            'A:/qiusiqi_1_20260122_053150.mff',
            'A:/qiusiqi_2_20260122_060138.mff',
            'A:/qiusiqi_3_20260122_062737.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    },
    'machenxiang': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/machenxiang',
        'mff_folders': [
            'A:/machenxiang_1_20260122_092234.mff',
            'A:/machenxiang_2_20260122_094915.mff',
            'A:/machenxiang_3_20260122_101417.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-2-{session}',
        'missing_trials': None
    },
    'lizhuhang': {
        'processed_data_path': 'A:/standard_data_interp_ica_99/lizhuhang',
        'mff_folders': [
            'A:/lizhuhang_1_20260123_100230.mff',
            'A:/lizhuhang_2_20260123_103106.mff',
            'A:/lizhuhang_3_20260123_105925.mff'
        ],
        'benchmark_pattern': 'benchmark_1_10-1-{session}',
        'missing_trials': None
    }
...
for _subject_name, _subject_cfg in SUBJECTS_CONFIG.items():
    _subject_cfg['processed_data_path'] = os.path.join(EEG_DATA_BASE_DIR, _subject_name)

# 实验音频与映射（所有被试共享）
# 映射 csv：benchmark 里的 a001.wav -> original_name / original_path
MAPPING_BASE_DIR = r"D:\D\research\audioset下载\audio_pairs_2s\分组音频"
STEREO_PAIRS_DIR = r"D:\D\research\audioset下载\audio_pairs_2s\subcategory_pairs"
MAPPING_FILES = [
...
# 仅作 stereo_path_map 缺失时的回退（行为/旧脚本用 clap_select 5s 单耳）
AUDIO_BASE_DIR = DEFAULT_AUDIO_BASE_DIR
AUDIO_SOURCE_TAG = "audio_pairs_2s_stereo"
...
# 是否在音频特征中追加预计算的 wav2vec 惊喜度特征（mse_sum_diff，一对音频一个标量）
USE_WAV2VEC_SURPRISE = False

# 本脚本可训练的模型（在 __main__ 或 main/main_individual 中通过 models_to_train 覆盖）
# 可选: simple, audio_only, residual, residual_eeg_only,
#       residual_eeg_cross_single, residual_eeg_cross_dual, residual_sync
MODELS_TO_TRAIN = ['audio_only', 'residual']

RESIDUAL_L2_MODEL_NAMES = {
    'residual', 'residual_eeg_only', 'residual_eeg_cross_single',
    'residual_eeg_cross_dual', 'residual_sync',
}


def normalize_models_to_train(models_to_train=None):
    """校验并规范化要训练的模型列表（保持顺序、去重）。"""
    if models_to_train is None:
        models_to_train = MODELS_TO_TRAIN
    if not models_to_train:
        raise ValueError("models_to_train 不能为空")

    allowed = {
        'simple', 'audio_only', 'residual', 'residual_eeg_only',
        'residual_eeg_cross_single', 'residual_eeg_cross_dual', 'residual_sync',
    }
    normalized = list(dict.fromkeys(models_to_train))
    unknown = [m for m in normalized if m not in allowed]
    if unknown:
        raise ValueError(
            f"未知模型: {unknown}。可选: {sorted(allowed)}"
        )
    return normalized


def resolve_subjects_to_run(subjects_to_run=None, subject_name=None):
    """解析要运行的被试列表（校验名称、去重、保持顺序）。

    优先级: subjects_to_run > subject_name > SUBJECTS_CONFIG 全部被试
    """
    if subjects_to_run:
        normalized = list(dict.fromkeys(subjects_to_run))
    elif subject_name:
        normalized = [subject_name]
    else:
        normalized = list(SUBJECTS_CONFIG.keys())

    unknown = [s for s in normalized if s not in SUBJECTS_CONFIG]
    if unknown:
        raise ValueError(
            f"未知被试: {unknown}。可用: {list(SUBJECTS_CONFIG.keys())}"
        )
    return normalized


def build_fold_models(eeg_channels, eeg_samples, audio_dim, sync_dim, use_sync, models_to_train):
    """按 models_to_train 构建当前 fold 需要训练的模型子集。"""
    models_to_train = normalize_models_to_train(models_to_train)
    all_models = {
        'simple': SimpleFusionClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim
        ),
        'audio_only': AudioOnlyClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim
        ),
        'residual': ResidualFusionClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim
        ),
        'residual_eeg_only': ResidualEEGFusionClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim
        ),
        ...
        'residual_eeg_cross_dual': ResidualEEGFusionCrossAttnDualQuery(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            ear_feature_dim=audio_dim // 2,
            audio_dim=audio_dim
        ),
    }
    if use_sync:
        all_models['residual_sync'] = ResidualSyncFusionClassifier(
            eeg_channels=eeg_channels,
            eeg_samples=eeg_samples,
            audio_dim=audio_dim,
            sync_dim=sync_dim
    ...
    for name in models_to_train:
        if name == 'residual_sync' and not use_sync:
            print(f"    跳过 {name}: 当前未启用同步性特征", flush=True)
            continue
        if name not in all_models:
            raise ValueError(f"模型 {name} 不可用（use_sync={use_sync}）")
        models[name] = all_models[name]

    if not models:
        raise ValueError(
            f"MODELS_TO_TRAIN={models_to_train} 在当前配置下没有可训练模型"
        )
    return models


def parse_audio_mapping_custom(file_path):
    """解析映射 csv，返回 {a001.wav: original_name}。"""
    return parse_audio_mapping_full(file_path)[0]


def parse_audio_mapping_full(file_path, played_folder_dir=None):
    """
    解析音频映射 csv。

    返回:
        name_map: {new_name: original_name}
        pair_to_stereo_path: {original_name: 2s 立体声 wav 绝对路径}
        played_path_map: {new_name: 分组音频 folder 内实际播放路径}（若提供 played_folder_dir）
    """
    try:
        ...
        if 'new_name' in df.columns and 'original_name' in df.columns:
            rows = df[['new_name', 'original_name'] + (['original_path'] if 'original_path' in df.columns else [])]
            for _, row in rows.iterrows():
                ...
                if new_name.lower().endswith('.wav'):
                    name_map[new_name[:-4]] = original_name

                stereo_path = None
                if 'original_path' in df.columns and not pd.isna(row.get('original_path')):
                    stereo_path = str(row['original_path']).strip()
                if not stereo_path or not os.path.exists(stereo_path):
                    stereo_path = os.path.join(STEREO_PAIRS_DIR, original_name)
                if os.path.exists(stereo_path):
                    pair_to_stereo_path[original_name] = stereo_path

                if played_folder_dir:
                    played_path = os.path.join(played_folder_dir, new_name)
                    if os.path.exists(played_path):
                        played_path_map[new_name] = played_path
        ...
        return name_map, pair_to_stereo_path, played_path_map
...
def load_pair_stereo_paths():
    """从三个 folder 的 mapping csv 汇总 pair_name -> 2s 立体声路径。"""
    pair_to_stereo_path = {}
    for folder_idx, mapping_file in enumerate(MAPPING_FILES):
        played_folder = os.path.join(MAPPING_BASE_DIR, f"folder{folder_idx + 1}")
        _, path_map, _ = parse_audio_mapping_full(mapping_file, played_folder_dir=played_folder)
        pair_to_stereo_path.update(path_map)
    print(f"已加载 {len(pair_to_stereo_path)} 个音频对的立体声路径（{STEREO_PAIRS_DIR}）")
    return pair_to_stereo_path
...
class ResidualFusionClassifier(nn.Module):
    ...
    思路：
    - 先用音频分支给出基础决策 logits_audio
    - 再用 EEG(+音频) 预测残差 delta
    - 最终 logits_final = logits_audio + delta
    """

    def __init__(self, eeg_channels, eeg_samples, audio_dim, hidden_dim=64, n_classes=2):
        super(ResidualFusionClassifier, self).__init__()

        # EEG特征提取器（可训练）
        self.eeg_extractor = EEGNetFeatureExtractor(eeg_channels, eeg_samples)

        # 音频分支（结构与 AudioOnlyClassifier 类似）
        ...
        # 残差分支：输入 EEG 特征 + 音频特征，输出与 logits_audio 同维度的残差
    ...
    def forward(self, eeg_data, audio_features, return_parts=False):
        """
        eeg_data: (batch, 1, n_channels, n_timepoints)
        audio_features: (batch, audio_dim)
        如果 return_parts=True，则返回 (logits_final, logits_audio, delta)
        """
        # 提取EEG特征
        eeg_feat = self.eeg_extractor(eeg_data)  # (batch, 160)

        # 音频基础决策
        logits_audio = self.audio_head(audio_features)  # (batch, n_classes)

        # 残差预测
        combined = torch.cat([eeg_feat, audio_features], dim=1)  # (batch, 160+audio_dim)
        delta = self.residual_head(combined)  # (batch, n_classes)

        logits_final = logits_audio + delta

        if return_parts:
            return logits_final, logits_audio, delta
        return logits_final


class ResidualSyncFusionClassifier(nn.Module):
    """
    三层残差融合分类模型（端到端版本）

    Layer 1：音频基础决策（stimulus-driven）
        audio_features → audio_head → logits_audio

    Layer 2：EEG 残差（状态修正，被动注意）
        eeg_data → EEGNet → eeg_feat
        [eeg_feat, audio_features] → residual_head_eeg → delta_eeg
        logits_1 = logits_audio + delta_eeg

    Layer 3：同步性残差（cross-modal coupling 修正）
        sync_features (PLV等) → residual_head_sync → delta_sync
        logits_final = logits_1 + delta_sync
    """

    def __init__(self, eeg_channels, eeg_samples, audio_dim, sync_dim=5, hidden_dim=64, n_classes=2):
        super(ResidualSyncFusionClassifier, self).__init__()
        ...
        # Layer 2: EEG 特征提取 + EEG 残差分支
        self.eeg_extractor = EEGNetFeatureExtractor(eeg_channels, eeg_samples)
        eeg_audio_dim = 160 + audio_dim
        self.residual_head_eeg = nn.Sequential(
            nn.Linear(eeg_audio_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim, n_classes)
        )

        # Layer 3: 同步性残差分支（只预测第二层之上的残差）
        self.residual_head_sync = nn.Sequential(
    ...
    def forward(self, eeg_data, audio_features, sync_features, return_parts=False):
        """
        eeg_data: (batch, 1, n_channels, n_timepoints)
        audio_features: (batch, audio_dim)
        sync_features: (batch, sync_dim) - 同步性特征（PLV等）
        如果 return_parts=True，则返回 (logits_final, logits_audio, delta_eeg, delta_sync)
        """
        # Layer 1: 音频基础决策
        logits_audio = self.audio_head(audio_features)  # (batch, n_classes)
        ...
        # Layer 2: EEG 残差
        eeg_feat = self.eeg_extractor(eeg_data)  # (batch, 160)
        combined_eeg_audio = torch.cat([eeg_feat, audio_features], dim=1)  # (batch, 160+audio_dim)
        delta_eeg = self.residual_head_eeg(combined_eeg_audio)  # (batch, n_classes)
        logits_1 = logits_audio + delta_eeg

        # Layer 3: 同步性残差（在第二层之上的修正）
        delta_sync = self.residual_head_sync(sync_features)  # (batch, n_classes)
        logits_final = logits_1 + delta_sync

        if return_parts:
            return logits_final, logits_audio, delta_eeg, delta_sync
        return logits_final
...
def load_trial_audio_pair(wavfile, base_dir=None, stereo_path=None, stereo_path_map=None):
    """
    加载 trial 对应的左右耳音频（优先 2s 立体声实验文件）。

    参数:
        wavfile: pair_name 或 a001.wav
        base_dir: clap_select 回退目录
        stereo_path: 直接指定 2s 立体声 wav
        stereo_path_map: {pair_name: stereo_path}
    """
    try:
        if stereo_path is None and stereo_path_map is not None:
            stereo_path = stereo_path_map.get(wavfile)
            if stereo_path is None and isinstance(wavfile, str):
                stereo_path = stereo_path_map.get(wavfile.replace('.wav', ''))

        if stereo_path and os.path.exists(stereo_path):
            return load_stereo_2s(stereo_path)
...
class SynchronizationFeatureExtractor:
    """
    同步性特征提取器
    计算EEG与左右音频的同步性特征向量（仅PLV：每个5维，5个频段）
    """
    ...
    def extract_sync_features(self, eeg_data, pair_name, audio_base_dir, stereo_path_map=None, verbose=False):
        """
        提取同步性特征
        ...
        返回:
            (left_sync_vector, right_sync_vector) - 每个都是5维向量（5个频段PLV）
        """
        # 默认返回零向量
        zero_vector = np.zeros(5, dtype=np.float32)  # 仅PLV：5个频段

        # 加载音频
        yL, yR, sr = load_trial_audio_pair(
            pair_name, base_dir=audio_base_dir, stereo_path_map=stereo_path_map
        )
        ...
        try:
            ...
            # 仅PLV：5维向量 [plv_delta, plv_theta, plv_alpha, plv_beta, plv_gamma]
            left_sync_vector = np.array(left_plvs, dtype=np.float32)
            right_sync_vector = np.array(right_plvs, dtype=np.float32)
...
def compute_sync_features_cache(X_eeg_raw, consistent_trials, eeg_sampling_rate=250,
                                audio_base_dir=None, output_dir=None, stereo_path_map=None):
    """
    预计算所有trial的同步性特征并缓存
    ...
    返回:
        sync_features: 同步性特征数组 (n_trials, 5) - 左右PLV相减（每频段1维）
        cache_path: 缓存文件路径
    """
    if audio_base_dir is None:
        audio_base_dir = AUDIO_BASE_DIR
    if stereo_path_map is None:
    ...
    if os.path.exists(cache_path) and os.path.exists(cache_info_path):
        ...
        try:
            ...
            # 验证缓存是否匹配当前数据
            if sync_features.shape[0] == len(consistent_trials) and sync_features.shape[1] == 5:
    ...
    print(f"  每个trial需要：加载音频文件 + 计算5个频段PLV（仅PLV，不含相关性）")
    ...
    for i, trial in enumerate(consistent_trials):
        ...
        # 计算同步性特征
        stereo_path = trial.get('stereo_path') or stereo_path_map.get(pair_name)
        left_sync, right_sync = sync_extractor.extract_sync_features(
            eeg_sample, pair_name, audio_base_dir,
            stereo_path_map=stereo_path_map if stereo_path is None else {pair_name: stereo_path},
        )
        ...
        # 左右 PLV 相减：5维 [plv_L_delta - plv_R_delta, ...]，表示左右耳耦合不对称
        sync_feat = np.array(left_sync, dtype=np.float32) - np.array(right_sync, dtype=np.float32)
        sync_features_list.append(sync_feat)

    # 转换为numpy数组
    sync_features = np.stack(sync_features_list, axis=0)  # (n_trials, 5)
...
def load_all_subjects_data():
    ...
    pair_stereo_paths = load_pair_stereo_paths()
    ...
    for subject_name, config in SUBJECTS_CONFIG.items():
        ...
        try:
            ...
            for session_idx, benchmark_file in enumerate(benchmark_files):
                ...
                played_folder = os.path.join(MAPPING_BASE_DIR, f"folder{session_idx + 1}")
                _, session_path_map, played_path_map = parse_audio_mapping_full(
                    MAPPING_FILES[session_idx], played_folder_dir=played_folder
                )
                ...
                for trial in parsed_trials:
                    ...
                    stereo_path = session_path_map.get(original_name) or pair_stereo_paths.get(original_name)
                    played_audio_path = played_path_map.get(filename) or os.path.join(played_folder, filename)
                    ...
                    parsed_trials_all_sessions.append({
                        ...
                        'played_audio_path': played_audio_path,
                    })
...
def extract_audio_features(consistent_trials, project_root, stereo_path_map=None):
    """
    使用IndependentEarComparisonModel提取音频特征
    ...
    base_dataset = FeatureTimeSeriesDataset(
        ...
        window_size=2,
        use_high_res=False,
        stereo_path_map=stereo_path_map,
    )
...
def build_pair_consistent_folds(consistent_trials, n_folds=5, seed=42):
    ...
    n_paired_keys = sum(1 for inds in pair_key_to_indices.values() if len(inds) == 2)
    n_single_keys = sum(1 for inds in pair_key_to_indices.values() if len(inds) == 1)
    n_multi_keys = sum(1 for inds in pair_key_to_indices.values() if len(inds) > 2)
    print(
        f"  [parse_pair_name 规范化键] 含 AB+BA 两 trial 的键: {n_paired_keys}; "
        f"仅单版本 trial 的键: {n_single_keys}; "
        f"同一键下 >2 个 trial: {n_multi_keys}"
    )
    ...
    idx_to_fold = {}
    for fi, fold_arr in enumerate(folds):
        for idx in fold_arr:
            idx_to_fold[int(idx)] = fi

    max_show = 12
    print(f"  [左右平衡对 → fold] 示例（parse_pair_name 键一致则视为同一对刺激，最多 {max_show} 条）:")
    shown = 0
    for key, indices in sorted(pair_key_to_indices.items(), key=lambda x: str(x[0])):
        if len(indices) != 2:
            continue
        if shown >= max_show:
            break
        f0 = idx_to_fold.get(int(indices[0]), -1)
        f1 = idx_to_fold.get(int(indices[1]), -1)
        ok = f0 == f1
        pn0 = consistent_trials[indices[0]].get("pair_name", "")
        pn1 = consistent_trials[indices[1]].get("pair_name", "")
        print(f"    key={key}")
        print(f"      fold={f0} vs {f1}  {'同折' if ok else '不同折'}")
        print(f"      trial[{indices[0]}] {pn0}")
        print(f"      trial[{indices[1]}] {pn1}")
        shown += 1
    if n_paired_keys > max_show:
        print(f"    ... 另有 {n_paired_keys - max_show} 个成对键未逐条打印")
...
def train_and_evaluate(X_eeg, X_left_audio, X_right_audio, y, n_folds=5, seed=42):
    ...
    for fold_idx, test_idx in enumerate(folds, 1):
        ...
        for model_name, model in models.items():
            ...
            print(
                f"  {model_name:12s} - 准确率: {val_acc:.4f}, 平衡准确率: {val_balanced_acc:.4f}, AUC: {val_auc:.4f}, Epochs: {actual_epochs}")
...
def train_and_evaluate_all_models(X_eeg_raw, X_audio_manual, y, subject_ids,
    ...
    for fold_idx, test_idx in enumerate(folds, 1):
        ...
        for model_name, model in models.items():
            ...
            # Batch处理配置
            batch_size = 64 if model_name in ['sync_fusion', 'nme_fusion'] else 64  # 同步性模型和NME模型用更小的batch
            n_train = len(y_train)
                    ...
                    with torch.no_grad():
                        ...
                        if model_name == 'nme_fusion':
                            ...
                            with torch.no_grad():
                                ...
                                log_var_mean = (
                                                           model.log_intercept_variance.data.item() + model.log_slope_variance.data.mean().item()) / 2
    ...
    for model_name, result_dict in all_results.items():
        ...
        print(
            f"{model_display_name:<20}{result_dict['mean_acc']:<10.4f}{result_dict['mean_balanced_acc']:<12.4f}{result_dict['mean_auc']:<10.4f}{mean_epochs:<8.1f}")
...
def collect_fold_data_from_global_indices(subject_names, subject_consistent_trials,
    ...
    return {
        ...
        'train_global_indices': train_global_indices,  # 添加全局索引，用于同步性特征
        'val_eeg': val_eeg_all,
        'val_labels': val_labels_all,
        'val_subjects': val_subjects_all,
        'val_pair_indices': val_pair_indices,
        'val_global_indices': val_global_indices  # 添加全局索引，用于同步性特征
    }
...
def train_fold_model(fold_data, all_audio_features, pair_name_to_idx, pair_names,
                     all_sync_features=None, models_to_train=None):
    """
    在单个fold上训练模型并返回评估结果
    
    参数:
        all_sync_features: (n_total_trials, sync_dim) - 所有trial的同步性特征缓存，如果为None则不使用同步性特征
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # 确保标签为 0/1，否则 CrossEntropyLoss 会触发 CUDA assert (t>=0 && t<n_classes)
    ...
    # 选择同步性特征（如果有）
    if all_sync_features is not None and train_global_indices is not None and val_global_indices is not None:
        X_sync_train = all_sync_features[train_global_indices]
        X_sync_val = all_sync_features[val_global_indices]
        sync_dim = all_sync_features.shape[1]
        use_sync = True
    else:
        X_sync_train = None
        X_sync_val = None
        sync_dim = 5   # 左右PLV相减，5个频段
        use_sync = False
    ...
    models = build_fold_models(
        eeg_channels, eeg_samples, audio_dim, sync_dim, use_sync, models_to_train
    )
    ...
    print(f"    开始训练 {len(models)} 个模型: {', '.join(model_names_list)}", flush=True)
    print(f"    设备: {device}, 训练样本: {n_train}, batch={batch_size}, 最多 {n_epochs} epochs", flush=True)
    print(f"    （每个 epoch 依次训练全部模型，请稍候…）", flush=True)
    ...
    if use_sync:
        X_sync_val_t = torch.FloatTensor(X_sync_val).to(device)
    ...
    for epoch in range(n_epochs):
        for mi, (name, model) in enumerate(models.items(), 1):
            model.train()
            epoch_loss = epoch_ce = epoch_reg = 0.0
            epoch_correct = epoch_total = 0

            if epoch == 0:
                print(f"    Epoch 1 训练中 [{mi}/{len(models)}] {name} ...", flush=True)
            ...
            for b in range(n_batches):
                ...
                if name == 'audio_only':
                    ...
                    loss = ce_loss + lambda_reg * dl2
                    ce = ce_loss.item()
                elif name == 'residual_sync' and use_sync:
                    Xs = torch.FloatTensor(X_sync_train[s:e]).to(device)
                    logits, _, de, ds = model(Xe, Xa, Xs, return_parts=True)
                    ce_loss = criterion(logits, yb)
                    r = torch.norm(de, p=2, dim=1).mean() + torch.norm(ds, p=2, dim=1).mean()
                    reg = r.item()
                    loss = ce_loss + lambda_reg * r
                    ce = ce_loss.item()
                elif name == 'residual_eeg_cross_single':
                    logits, _, delta = model(Xe, Xa, return_parts=True)
                    ce_loss = criterion(logits, yb)
                    dl2 = torch.norm(delta, p=2, dim=1).mean()
                    reg = dl2.item()
                    loss = ce_loss + lambda_reg * dl2
                    ce = ce_loss.item()
                elif name == 'residual_eeg_cross_dual':
                    # 简单地将音频特征按前后拆分为左右耳特征（如果实际结构不同，可在数据层单独提供）
                    ear_dim = Xa.shape[1] // 2
                    left_feat = Xa[:, :ear_dim]
                    right_feat = Xa[:, ear_dim:]
                    logits, _, delta = model(Xe, Xa, left_feat, right_feat, return_parts=True)
                    ce_loss = criterion(logits, yb)
                    dl2 = torch.norm(delta, p=2, dim=1).mean()
                    reg = dl2.item()
                    loss = ce_loss + lambda_reg * dl2
            ...
            if epoch == 0:
                ...
                print(f"    Epoch 1 [{name}] 训练完成, train_acc={ta:.4f}", flush=True)

        # 第 1 个 epoch 必打；之后每 5 个 epoch 打一次（原先是每 10 个）
        log_this_epoch = (epoch == 0) or ((epoch + 1) % 5 == 0)
        if log_this_epoch:
            for name, model in models.items():
                model.eval()
                with torch.no_grad():
                    if name == 'audio_only':
                        val_out = model(None, X_audio_val_t)
                    elif name == 'residual_sync' and use_sync:
                        val_out = model(X_eeg_val, X_audio_val_t, X_sync_val_t)
                    else:
                ...
                tl, tc, tr, ta = last_train[name]['loss'], last_train[name]['ce'], last_train[name]['reg'], last_train[name]['acc']
                if name in RESIDUAL_L2_MODEL_NAMES:
                    print(f"    Epoch {epoch + 1}/{n_epochs} [{name:12s}] Train Loss: {tl:.4f} (Cls: {tc:.4f}, Reg: {tr:.4f}), "
                          f"Train Acc: {ta:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}", flush=True)
                else:
                    print(f"    Epoch {epoch + 1}/{n_epochs} [{name:12s}] Train Loss: {tl:.4f}, Train Acc: {ta:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}", flush=True)

            if all(patience_counters[n] >= patience for n in models.keys()):
                break

    actual_epochs = epoch + 1
    print(f"    实际训练了 {actual_epochs} 个 epoch", flush=True)
    ...
    with torch.no_grad():
        for name, model in models.items():
            ...
            print(f"    {name:12s} - 准确率: {acc:.4f}, 平衡准确率: {bal_acc:.4f}, AUC: {auc:.4f}, Epochs: {actual_epochs}", flush=True)

    return metrics


def save_cross_validation_results(all_fold_results, subject_names, per_subject_mean=None, model_names=None):
    """
    保存交叉验证结果。可选写入各被试个人准确率（per_subject_mean）。
    """
    model_names = model_names or ['simple', 'audio_only', 'residual']
    if any('val_accuracy_residual_sync' in result for result in all_fold_results) and 'residual_sync' not in model_names:
        model_names = list(model_names) + ['residual_sync']

    # CSV 不保存 per_subject 的 dict 列，避免序列化复杂
    to_csv = []
    for r in all_fold_results:
        row = {k: v for k, v in r.items() if not k.startswith('val_accuracy_per_subject_')}
        to_csv.append(row)
    results_df = pd.DataFrame(to_csv)
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"各折结果（{' / '.join(model_names)}）:\n")
        for result in all_fold_results:
            result_line = f"  Fold {result['fold_idx']}: "
            for name in model_names:
                result_line += f"{name}_acc={result[f'val_accuracy_{name}']:.4f}, "
            f.write(result_line.rstrip(', ') + "\n")

        f.write(f"\n{'=' * 60}\n")
        for name in model_names:
            accuracies = [result[f'val_accuracy_{name}'] for result in all_fold_results]
            balanced_accuracies = [result[f'val_balanced_accuracy_{name}'] for result in all_fold_results]
            aucs = [result[f'val_auc_{name}'] for result in all_fold_results]
        ...
        if per_subject_mean:
            f.write(f"\n{'=' * 60}\n")
            f.write("各被试个人准确率（跨 fold 平均）\n")
            f.write(f"{'=' * 60}\n")
            for name in model_names:
                if name not in per_subject_mean:
                    continue
                f.write(f"\n模型 {name}:\n")
                for sid in sorted(per_subject_mean[name].keys()):
                    sname = subject_names[sid] if sid < len(subject_names) else f"subject_{sid}"
                    f.write(f"  {sname}: {per_subject_mean[name][sid]:.4f}\n")

    print(f"汇总信息已保存: {summary_path}")


def _load_all_audio_features(project_root, pair_names, stereo_path_map=None):
    """
    加载或构建所有音频对的特征（多被试/单人模式共用同一缓存）。
    返回: all_audio_features, pair_names(可能与输入相同或缩短), pair_name_to_idx
    """
    ...
    if os.path.exists(features_cache_path) and os.path.exists(meta_cache_path):
        try:
            ...
            cached_has_wav2vec = cache_meta.get('has_wav2vec_surprise', False)
            cached_audio_source = cache_meta.get('audio_source', '')
            if (cached_pair_names == pair_names
                    and cached_audio_source == AUDIO_SOURCE_TAG
                    and (not USE_WAV2VEC_SURPRISE or cached_has_wav2vec)):
                ...
                if USE_WAV2VEC_SURPRISE and cached_has_wav2vec:
                    print("缓存中已包含 wav2vec 惊喜度特征 (mse_sum_diff)。")
        except Exception as e:
    ...
    if all_audio_features is None:
        ...
        left_audio_features, right_audio_features, feature_mapping = extract_audio_features(
            all_audio_trials, project_root, stereo_path_map=stereo_path_map
        )
        if len(left_audio_features) != len(pair_names):
            print(f"警告: 只有 {len(left_audio_features)}/{len(pair_names)} 个音频对成功提取特征")
            pair_names = feature_mapping['pair_names']
            all_audio_features = np.concatenate([left_audio_features, right_audio_features], axis=1)
        else:
            all_audio_features = np.concatenate([left_audio_features, right_audio_features], axis=1)

        has_wav2vec = False
        if USE_WAV2VEC_SURPRISE:
            print("\n=== 追加 wav2vec 惊喜度特征 (mse_sum_diff) 到音频特征 ===")
            ...
            if wav_info is not None and wav_info.get('zscore') is not None:
                ...
                all_audio_features = np.concatenate([all_audio_features, wav_z], axis=1)
                has_wav2vec = True
                print(f"已追加 wav2vec 特征列，新的音频特征形状: {all_audio_features.shape}")
            else:
                print("警告: 未能加载有效的 wav2vec mse_sum_diff 特征，保持原有音频特征。")

        np.save(features_cache_path, all_audio_features)
        cache_meta = {
            'pair_names': pair_names,
            'n_pairs': len(pair_names),
            'feature_shape': list(all_audio_features.shape),
            'cache_time': pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S'),
            'has_wav2vec_surprise': has_wav2vec,
            'audio_source': AUDIO_SOURCE_TAG,
        }
    ...
    pair_name_to_idx = {pn: idx for idx, pn in enumerate(pair_names)}
    return all_audio_features, pair_names, pair_name_to_idx


def align_single_subject_data(subject_name, filter_consistent=True, project_root=None):
    """
    加载并对齐单个被试的 EEG、标签与一致 trials。
    返回: consistent_eeg, consistent_labels, consistent_trials, meta, pair_names
    """
    if project_root is None:
        project_root = os.getcwd()
    if subject_name not in SUBJECTS_CONFIG:
        raise ValueError(f"未知被试: {subject_name}. 可用: {list(SUBJECTS_CONFIG.keys())}")
    ...
    if filter_consistent:
        ...
        print(f"  所有 trials: {len(consistent_trials)}")

    if len(consistent_trials) == 0:
        raise RuntimeError(f"被试 {subject_name} 没有有效 trials")
    ...
    for trial in consistent_trials:
        ...
        if idx >= len(eeg_data):
            continue
        consistent_eeg.append(eeg_data[idx])
        consistent_labels.append(int(c) - 1)
        filtered_trials.append(trial)

    consistent_eeg = np.array(consistent_eeg)
    consistent_labels = np.array(consistent_labels)
    if len(consistent_eeg) == 0:
        raise RuntimeError(f"被试 {subject_name} 过滤后无有效 trials（choice 需为 1/2）")

    print(f"  对齐后: {len(consistent_eeg)} trials, EEG 形状 {consistent_eeg.shape}")
    print(f"  标签分布: 选A={np.sum(consistent_labels == 0)}, 选B={np.sum(consistent_labels == 1)}")
    return consistent_eeg, consistent_labels, filtered_trials, meta, pair_names


def _select_audio_for_trials(consistent_eeg, consistent_labels, consistent_trials,
                             all_audio_features, pair_name_to_idx):
    """按 pair_name 对齐音频特征，丢弃无音频特征的 trial。"""
    sel_eeg, sel_labels, sel_trials, sel_pair_idx = [], [], [], []
    for i, trial in enumerate(consistent_trials):
        pn = trial['pair_name']
        if pn not in pair_name_to_idx:
            continue
        sel_eeg.append(consistent_eeg[i])
        sel_labels.append(consistent_labels[i])
        sel_trials.append(trial)
        sel_pair_idx.append(pair_name_to_idx[pn])

    if not sel_eeg:
        raise RuntimeError("没有 trial 能匹配到音频特征")

    return (
        np.array(sel_eeg),
        np.array(sel_labels),
        sel_trials,
        np.array(sel_pair_idx, dtype=np.int32),
    )


def _build_single_subject_fold_data(consistent_eeg, consistent_labels, pair_indices,
                                    train_idx, val_idx):
    """构造 train_fold_model 所需的 fold_data（单人版）。"""
    return {
        'train_eeg': consistent_eeg[train_idx],
        'train_labels': consistent_labels[train_idx],
        'train_pair_indices': pair_indices[train_idx],
        'val_eeg': consistent_eeg[val_idx],
        'val_labels': consistent_labels[val_idx],
        'val_pair_indices': pair_indices[val_idx],
        'val_subjects': np.zeros(len(val_idx), dtype=np.int32),
    }


def _detect_model_names_from_results(all_fold_results):
    """从 fold 结果中自动识别已训练的模型名。"""
    names = set()
    for r in all_fold_results:
        for k in r:
            if k.startswith('val_accuracy_') and not k.startswith('val_accuracy_per_subject_'):
                names.add(k.replace('val_accuracy_', ''))
    preferred = [
        'simple', 'audio_only', 'residual', 'residual_eeg_only',
        'residual_eeg_cross_single', 'residual_eeg_cross_dual', 'residual_sync',
    ]
    ...
    return ordered or ['simple', 'audio_only', 'residual']


def save_individual_cv_results(subject_name, all_fold_results, output_dir, model_names=None):
    """保存单人交叉验证结果。"""
    os.makedirs(output_dir, exist_ok=True)
    model_names = model_names or _detect_model_names_from_results(all_fold_results)

    to_csv = []
    for r in all_fold_results:
        row = {k: v for k, v in r.items() if not k.startswith('val_accuracy_per_subject_')}
        to_csv.append(row)
    results_path = os.path.join(output_dir, f"{subject_name}_residual_cv_results.csv")
    pd.DataFrame(to_csv).to_csv(results_path, index=False, encoding='utf-8-sig')

    summary_path = os.path.join(output_dir, f"{subject_name}_residual_cv_summary.txt")
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write(f"单人 EEG 音频残差分类 - {subject_name}\n")
        f.write("=" * 60 + "\n")
        f.write(f"交叉验证折数: {len(all_fold_results)}\n\n")
        for result in all_fold_results:
            line = f"  Fold {result['fold_idx']}: "
            line += ", ".join(f"{n}={result[f'val_accuracy_{n}']:.4f}" for n in model_names if f'val_accuracy_{n}' in result)
            f.write(line + "\n")
        f.write(f"\n{'=' * 60}\n")
        for name in model_names:
            key = f'val_accuracy_{name}'
            if not any(key in r for r in all_fold_results):
                continue
            accs = [r[key] for r in all_fold_results if key in r]
            bal_key = f'val_balanced_accuracy_{name}'
            auc_key = f'val_auc_{name}'
            bals = [r[bal_key] for r in all_fold_results if bal_key in r]
            aucs = [r[auc_key] for r in all_fold_results if auc_key in r]
            f.write(f"模型 {name}:\n")
            f.write(f"  平均验证准确率: {np.mean(accs):.4f} ± {np.std(accs):.4f}\n")
            if bals:
                f.write(f"  平均平衡准确率: {np.mean(bals):.4f} ± {np.std(bals):.4f}\n")
            if aucs:
                f.write(f"  平均AUC: {np.mean(aucs):.4f} ± {np.std(aucs):.4f}\n")
            f.write("-" * 40 + "\n")
...
def run_single_subject_residual_cv(subject_name, all_audio_features, pair_names, pair_name_to_idx,
                                   filter_consistent=True, project_root=None, output_dir=None,
                                   models_to_train=None):
    """对单个被试做 pair-consistent 5 折交叉验证（每人单独训练）。"""
    if project_root is None:
        project_root = os.getcwd()
    if output_dir is None:
    ...
    consistent_eeg, consistent_labels, consistent_trials, meta, _ = align_single_subject_data(
        subject_name, filter_consistent=filter_consistent, project_root=project_root
    )
    ...
    print(f"\n交叉验证: {N_FOLDS} 折 (被试 {subject_name}), 模型: {', '.join(models_to_train)}")

    all_fold_results = []
    for fold_idx, val_idx in enumerate(folds, 1):
        train_idx = np.setdiff1d(shuffled_indices, val_idx, assume_unique=True)
        print(f"\n--- Fold {fold_idx}/{N_FOLDS} (训练 {len(train_idx)}, 验证 {len(val_idx)}) ---")

        fold_data = _build_single_subject_fold_data(
            consistent_eeg, consistent_labels, pair_indices, train_idx, val_idx
        )
        fold_metrics = train_fold_model(
            fold_data, all_audio_features, pair_name_to_idx, pair_names,
            all_sync_features=None, models_to_train=models_to_train
        )
        all_fold_results.append({
        ...
        parts = [f"{n}={fold_metrics[f'val_accuracy_{n}']:.4f}"
                 for n in models_to_train
                 if f'val_accuracy_{n}' in fold_metrics]
        print(f"  Fold {fold_idx} 结果: {', '.join(parts)}")

    model_names = _detect_model_names_from_results(all_fold_results)
    print(f"\n{'=' * 60}")
    print(f"被试 {subject_name} 交叉验证汇总:")
    for name in model_names:
        accs = [r[f'val_accuracy_{name}'] for r in all_fold_results if f'val_accuracy_{name}' in r]
        if accs:
            print(f"  {name}: {np.mean(accs):.4f} ± {np.std(accs):.4f}")

    save_individual_cv_results(subject_name, all_fold_results, output_dir, model_names=model_names)
    return all_fold_results


def main_individual(subject_name=None, subjects_to_run=None, filter_consistent=True, models_to_train=None):
    """
    单人模式：每个被试单独划分 fold、单独训练模型（数据不跨被试混合）。

    参数:
        subject_name: 单个被试名（subjects_to_run 为空时生效）
        subjects_to_run: 被试名列表，例如 ['hanglei', 'yanxingzhuo']；优先于 subject_name
        filter_consistent: 是否只使用两次选择一致的 trials
        models_to_train: 要训练的模型列表，默认 MODELS_TO_TRAIN
    """
    models_to_train = normalize_models_to_train(models_to_train)
    subjects = resolve_subjects_to_run(subjects_to_run, subject_name)
    print("=" * 60)
    print("EEG 音频残差分类 - 单人 pair-wise 交叉验证")
    print(f"训练模型: {', '.join(models_to_train)}")
    print(f"被试 ({len(subjects)}): {', '.join(subjects)}")
    if filter_consistent:
        print("每个被试单独筛选一致 trials，各自训练模型")
    else:
        print("每个被试使用全部 trials，各自训练模型")
    print("=" * 60)
    ...
    print(f"\n音频对总数: {len(pair_names)}")
    all_audio_features, pair_names, pair_name_to_idx = _load_all_audio_features(
        project_root, pair_names, stereo_path_map=stereo_path_map
    )
    ...
    for subj in subjects:
        try:
            fold_results = run_single_subject_residual_cv(
                subj, all_audio_features, pair_names, pair_name_to_idx,
                filter_consistent=filter_consistent,
                project_root=project_root,
                output_dir=individual_output_dir,
                models_to_train=models_to_train,
            )
            model_names = _detect_model_names_from_results(fold_results)
            row = {'subject': subj}
            for name in model_names:
                accs = [r[f'val_accuracy_{name}'] for r in fold_results if f'val_accuracy_{name}' in r]
                if accs:
                    row[f'mean_acc_{name}'] = float(np.mean(accs))
    ...
    if all_subjects_summary:
        ...
        summary_path = os.path.join(individual_output_dir, 'all_subjects_summary.csv')
        summary_df.to_csv(summary_path, index=False, encoding='utf-8-sig')
        print(f"\n所有被试汇总已保存: {summary_path}")


def main(filter_consistent=True, models_to_train=None):
    """
    ...
    for subject_name in subject_names:
        ...
        # 对齐EEG数据和consistent_trials，仅保留 choice/response 为 1 或 2 的 trial（避免 CrossEntropyLoss 报错）
        ...
        print(
            f"  标签分布: 选择A: {np.sum(subject_consistent_labels == 0)}, 选择B: {np.sum(subject_consistent_labels == 1)}")
    ...
    if os.path.exists(features_cache_path) and os.path.exists(meta_cache_path):
        try:
            ...
            # 验证pair_names是否匹配，且（如启用）缓存中已包含wav2vec特征
            if (cached_pair_names == pair_names
                    and cached_audio_source == AUDIO_SOURCE_TAG
                    and (not USE_WAV2VEC_SURPRISE or cached_has_wav2vec)):
                all_audio_features = np.load(features_cache_path)
                ...
                elif USE_WAV2VEC_SURPRISE and not cached_has_wav2vec:
                    print("缓存未包含 wav2vec 惊喜度特征，重新提取以追加该特征...")
                all_audio_features = None
        except Exception as e:
            print(f"缓存文件损坏，重新提取: {e}")
            all_audio_features = None
    ...
    if all_audio_features is None:
        ...
        # 如果启用，在此处追加 wav2vec 惊喜度特征 (mse_sum_diff)
        has_wav2vec =True
        if USE_WAV2VEC_SURPRISE:
            print("\n=== 追加 wav2vec 惊喜度特征 (mse_sum_diff) 到音频特征 ===")
            wav_info = load_wav2vec_feature_zscore(
            ...
            if wav_info is not None and wav_info.get('zscore') is not None:
                wav_z = np.asarray(wav_info['zscore'], dtype=float)
                if wav_z.shape[0] != len(pair_names):
                    print(f"警告: wav2vec 特征长度 {wav_z.shape[0]} 与 pair 数量 {len(pair_names)} 不一致，将对齐前者的长度")
                    min_len = min(len(pair_names), wav_z.shape[0])
                    wav_z = wav_z[:min_len]
                    all_audio_features = all_audio_features[:min_len]
                # 用0替换 NaN/Inf，0 对应全局均值 (z-score)
                invalid_mask = ~np.isfinite(wav_z)
                if invalid_mask.any():
                    print(f"警告: wav2vec mse_sum_diff 对 {invalid_mask.sum()} 个 pair 缺失/非有限值，将用 0 填充")
                    wav_z[invalid_mask] = 0.0
    ...
    # 5. 计算或加载同步性特征缓存（暂时注释掉，不使用 residual_sync）
    # print(f"\n=== 计算同步性特征（PLV等） ===")
    # all_consistent_trials_ordered = []
    # all_eeg_data_ordered = []
    # for global_idx in shuffled_indices:
    #     found = False
    #     for subject_name in subject_names:
    #         if subject_name not in subject_consistent_trials:
    #             continue
    #         info = subject_trial_info[subject_name]
    #         offset, n_trials = info['offset'], info['n_trials']
    #         if offset <= global_idx < offset + n_trials:
    #             local_idx = global_idx - offset
    #             trials = subject_consistent_trials[subject_name]
    #             eeg_data = subject_eeg_data[subject_name]
    #             if local_idx < len(trials) and local_idx < len(eeg_data):
    #                 all_consistent_trials_ordered.append(trials[local_idx])
    #                 all_eeg_data_ordered.append(eeg_data[local_idx])
    #                 found = True
    #                 break
    #     if not found:
    #         print(f"警告: 全局索引 {global_idx} 未找到对应的trial")
    # all_eeg_data_ordered = np.array(all_eeg_data_ordered)
    # sync_cache_dir = os.path.join(OUTPUT_DIR, 'sync_features_cache')
    # os.makedirs(sync_cache_dir, exist_ok=True)
    # all_sync_features, sync_cache_path = compute_sync_features_cache(
    #     all_eeg_data_ordered, all_consistent_trials_ordered,
    #     eeg_sampling_rate=SAMPLING_RATE, audio_base_dir=AUDIO_BASE_DIR, output_dir=sync_cache_dir
    # )
    all_sync_features = None
    print("\n=== 同步性特征已注释，不使用 residual_sync ===")
    ...
    for fold_idx, val_global_indices in enumerate(folds, 1):
        ...
        fold_metrics = train_fold_model(
            fold_data, all_audio_features, pair_name_to_idx, pair_names,
            all_sync_features=all_sync_features, models_to_train=models_to_train
        )
    ...
    model_names = [n for n in models_to_train
                   if any(f'val_accuracy_{n}' in result for result in all_fold_results)]
    if not model_names:
        model_names = _detect_model_names_from_results(all_fold_results)
    ...
    # 各被试个人准确率：按被试聚合各 fold 的 val 准确率再取平均
    per_subject_acc_lists = {name: defaultdict(list) for name in model_names}
    for r in all_fold_results:
        for name in model_names:
            d = r.get(f'val_accuracy_per_subject_{name}', {})
            for sid, acc in d.items():
                per_subject_acc_lists[name][sid].append(acc)
    per_subject_mean = {
        name: {sid: float(np.mean(lst)) for sid, lst in per_subject_acc_lists[name].items()}
        for name in model_names
    }

    print(f"\n{'=' * 60}")
    print("各被试个人准确率（跨 fold 平均）:")
    print(f"{'=' * 60}")
    for name in model_names:
        print(f"\n模型 {name}:")
        for sid in sorted(per_subject_mean[name].keys()):
            sname = subject_names[sid] if sid < len(subject_names) else f"subject_{sid}"
            print(f"  {sname}: {per_subject_mean[name][sid]:.4f}")

    # 保存结果
    save_cross_validation_results(all_fold_results, subject_names, per_subject_mean=per_subject_mean, model_names=model_names)


if __name__ == "__main__":
    # ===== 设置参数 =====
    # RUN_MODE: 'pooled' = 所有人混训; 'individual' = 每人单独训练
    RUN_MODE = 'individual'  # 'pooled' 或 'individual'
    FILTER_CONSISTENT = False  # True: 只使用两次选择一致的trials; False: 使用所有trials
    SUBJECT_NAME = None  # 单个被试（SUBJECTS_TO_RUN 为空时生效）
    SUBJECTS_TO_RUN = [
         'chenxianwei','yeziyuan', 'zhangzhiyao',

    ]
    # 要训练的模型（按需增删）:
    # simple, audio_only, residual, residual_eeg_only,
    # residual_eeg_cross_single, residual_eeg_cross_dual, residual_sync
    MODELS_TO_TRAIN = ['audio_only', 'residual_eeg_only']

    print("=" * 60)
    if RUN_MODE == 'individual':
        print("EEG音频残差分类 - 单人 pair-wise 交叉验证")
    else:
        print("EEG音频残差分类 - 多被试 pair-wise 交叉验证（混训）")
    print(f"运行模式: {RUN_MODE}")
    print(f"筛选一致trials: {FILTER_CONSISTENT}")
    print(f"训练模型: {', '.join(MODELS_TO_TRAIN)}")
    if RUN_MODE == 'individual':
        try:
            _run_subjects = resolve_subjects_to_run(SUBJECTS_TO_RUN, SUBJECT_NAME)
            print(f"运行被试 ({len(_run_subjects)}): {', '.join(_run_subjects)}")
    ...
    if RUN_MODE == 'individual':
        ...
        main(filter_consistent=FILTER_CONSISTENT, models_to_train=MODELS_TO_TRAIN)
