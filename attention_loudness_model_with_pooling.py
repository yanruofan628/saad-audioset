...
def extract_feature_time_series(y, sr, feature_type, hop_length=8000, compute_std=False, window_size=5):
    ...
    参数:
        ...
        hop_length: 帧移
        compute_std: 是否计算滑动窗口标准差（而不是原始时间序列）
        window_size: 滑动窗口大小（当compute_std=True时使用）
    ...
    if feature_type == 'loudness':
        ...
        # 注意：对于大hop_length（如8000），extract_time_series_features可能返回长度不一致的特征
        # 因此，我们统一使用256作为hop_length提取特征，然后再对齐到目标长度（mel_spec_len）
        # 这样可以确保所有特征都能正确提取，并且长度一致
        ...
        try:
            ...
            print(
                f"  错误: extract_time_series_features失败 (hop_length={effective_hop_length}, feature={feature_type}): {e}")
            import traceback
            traceback.print_exc()
            raise
    ...
    if original_len != mel_spec_len:
        if original_len > mel_spec_len:
            # 下采样：使用平均池化（优化版本，使用向量化操作）
            if mel_spec_len <= 0:
                raise ValueError(f"mel_spec_len必须大于0: {mel_spec_len}")

            # 使用更高效的方法：将原始序列分成mel_spec_len个区间，每个区间取平均值
            # 使用np.array_split然后对每个块求平均
            try:
                # 方法1：使用uniform_filter1d进行下采样（最快）
                if mel_spec_len > 1:
                    # 计算下采样因子
                    downsample_factor = original_len / mel_spec_len
                    # 使用uniform_filter1d进行平滑，然后每隔downsample_factor取一个点
                ...
                # 如果uniform_filter1d失败，使用备用方法：平均池化
    ...
    # 如果需要计算std，先提取原始时间序列，再计算滑动std
    # 注意：std计算应该在长度对齐之后进行，此时feat_series的长度应该是mel_spec_len
    if compute_std:
        try:
            feat_series = compute_rolling_std(feat_series, window_size=window_size)
            # std计算后长度应该保持不变，如果不一致则重新对齐
            if len(feat_series) != mel_spec_len:
                if len(feat_series) > mel_spec_len:
                    feat_series = feat_series[:mel_spec_len].copy()
                elif len(feat_series) < mel_spec_len:
...
def get_feature_configs_from_linear_regression():
    ...
    # 索引2: Spectral Centroid (std) -> spectral_centroid, compute_std=True
    # 索引7: Spectral Modulation Centroid (mean) -> spec_mod_centroid, compute_std=False
    # 索引8: Spectral Modulation Centroid (std) -> spec_mod_centroid, compute_std=True
    # 索引9: Spectral Irregularity (mean) -> spectral_irregularity, compute_std=False
    # 索引10: Spectral Irregularity (std) -> spectral_irregularity, compute_std=True
    # 索引12: Loudness (std) -> loudness, compute_std=True
    # 索引17: Temporal Modulation (mean) -> temporal_mod, compute_std=False

    # 根据extract_significant_features_from_table()返回的特征配置
    # 特征索引：2, 7, 8, 9, 10, 12, 17
    # 对应：Spectral Centroid (std),
    #      Spectral Modulation Centroid (mean), Spectral Modulation Centroid (std),
    #      Spectral Irregularity (mean), Spectral Irregularity (std),
    ...
    def _build(self):
        ...
        for i, pair_name in enumerate(self.pair_names):
            ...
            try:
                ...
                if T_cur <= 0:
                    raise ValueError(
                        f"特征长度为0或负数: T_cur={T_cur}, left_lens={[len(f) for f in left_features]}, right_lens={[len(f) for f in right_features]}")
                if T_ref is None:
                    T_ref = T_cur
                else:
...
def load_or_build_cached_dataset(feature_configs, project_root: str, base_dir: str, sr: int, hop_length: int,
                                 window_size: int = 5) -> FeatureTimeSeriesDataset:
    """
    构建/缓存多特征数据集：
    ...
    import hashlib
    # 生成缓存key（基于特征配置）
    feature_key = '_'.join([f"{ft}_{'std' if cs else 'mean'}" for ft, cs in feature_configs])
    ...
    if os.path.exists(cache_path):
        ...
        ds = FeatureTimeSeriesDataset(all_pair_names, base_dir, probs, feature_configs, sr=sr, hop_length=hop_length,
...
def train_model(model, train_dataset, val_dataset, epochs, device, output_dir,
    ...
    val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False,
                            drop_last=False) if val_dataset is not None else None
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loader is not None:
            ...
            if epoch % 10 == 0 or epoch == 1:
                print(
                    f"  Epoch {epoch}/{epochs} - train_loss={avg_train_loss:.6f} val_loss={avg_val_loss:.6f} (best={best_val_loss:.6f}, patience={patience_counter}/{patience})")
...
def analyze_attention_metrics(weights_dict, output_dir, model_type):
    ...
    if model_type == 'dual_stream' and 'left' in weights_dict and 'right' in weights_dict:
        ...
        if total > 0:
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'attention_curve_left_right_unified.png'), dpi=300,
                        bbox_inches='tight')
            plt.close()
...
def analyze_contribution(model, trials, base_dir, output_dir, feature_configs, T_ref, window_size=5):
    ...
    pd.DataFrame(rows_norm).to_csv(os.path.join(output_dir, 'contrib_metrics_k20_norm.csv'), index=False,
                                   encoding='utf-8-sig')
    ...
    pd.DataFrame(rows_raw).to_csv(os.path.join(output_dir, 'contrib_metrics_k20_raw.csv'), index=False,
                                  encoding='utf-8-sig')
...
def main():
    ...
    print(f"\n步骤2: 构建多特征数据集")
    window_size = 5
    dataset = load_or_build_cached_dataset(feature_configs, project_root, base_dir, sr=SR, hop_length=HOP,
                                           window_size=window_size)
    if len(dataset) == 0:
        print(f"  警告: 数据集为空，退出")
        return
    ...
    for model_type in model_types:
        ...
        for fold_idx, test_idx in enumerate(folds, start=1):
            ...
            train_model(model, train_ds_final, val_ds_final, epochs=80, device=device, output_dir=fold_out,
                        lr=5e-4, weight_decay=1e-4, patience=20)  # 降低学习率：1e-3 -> 5e-4，更稳定训练
